...
Findings are tracked as beads. There is no findings file in the tree: `docs/archive/findings.md` was deleted with `docs/archive/` at `ff04842e`, and the `wamn-at62` owner ruling of 2026-08-24 records the tracker as its live successor. File findings with `bd`; do not create a findings document.
...
Close findings in the commit that carries the finding ID (`fix(R13): ...`).
...
- `apps/{wamn_receiving,wamn_wms,client_acme_receiving}` holds each application manifest, SQL, generated output, component, and data library. Receiving also owns its composed operator in `apps/wamn_receiving/ui` and its command histories in `apps/wamn_receiving/tests`.
- `apps/platform/{events,execution,ingress,data}` holds shared wasm32-wasip2 guests and guest libraries. `apps/Cargo.toml` owns that workspace. The separate no_std workspace lives at `apps/platform/no-std`. Shared test guests live in `apps/platform/fixtures`. `components/nodes` went with the demo/POC surface (`wamn-0h0g.12.2`, `3554f140`).
...
- Per-bead build + gate-of-record commands: **`docs/operations/build-and-test.md`**.
