#!/usr/bin/env python3
"""Review the completed sixth workspace run against its recorded predecessors."""
...
for key, row in new_index.items():
    ...
    row['prior_classification_record'] = '../../run-005/classification-002/classified-failures.json'
    row['prior_source'] = old_source
    row['run5_comparison'] = 'Exact identity and normalized diagnostic match.'
...
new_unit_name = 'route_authentication_live::cluster::operator_recovery::tests::startup_refusal_retains_container_identity_during_restart_backoff'
assert added_cases == [('wamn_receiving_tests', 'unittests lib.rs', new_unit_name)]
assert not absent_cases
case_changes = {'added': [{**new_cases[key], 'result': case_result(key[2], description=key[1])} for key in added_cases],
    'absent': [], 'source_commit_for_added_test': subprocess.check_output(['git', 'rev-parse', 'c4c4c8b0'], cwd=root, text=True).strip()}
assert case_changes['added'][0]['result']['status'] == 'ok'
rename = read(prior_run / 'classification-002/renamed-test.json')
rename['prior_mapping_record'] = '../../run-005/classification-002/renamed-test.json'
assert tuple(rename['current_identity']) in new_index
assert new_index[tuple(rename['current_identity'])]['normalized_cause'] == rename['cause']
assert case_result(rename['current_identity'][2], description='unittests lib.rs')['status'] == 'FAILED'
assert source_bytes(rename['commit'] + '^', rename['path']).replace(rename['previous_identity'][2].split('::')[-1].encode(), rename['current_identity'][2].split('::')[-1].encode()) == source_bytes(rename['commit'], rename['path'])
check_source(rename['path'])
...
moves['source'] = source
moves['prior_mapping_record'] = '../../run-005/classification-002/app-test-moves.json'
...
for move in baseline_moves:
    ...
    move.update(current_cause=row['cause'], current_log_line=row['diagnostic_start_log_line'],
        prior_mapping_record='../../run-005/classification-002/baseline-moved-failures.json')
...
for item in value['pending_review']:
    if item['kind'] == 'current_failure_cause':
        ...
        resolutions.append({**item, 'classification': row['classification'], 'disposition': row['run5_comparison']})
    else:
...
value.update(classification_counts=dict(Counter(row['classification'] for row in value['failures'])),
    ...
    comparison_to_previous_run='run5-comparison.json', moved_failure_identities='baseline-moved-failures.json',
...
write('run5-comparison.json', comparison)
write('run5-case-changes.json', case_changes)
...
write('source-comparison.json', {'source': source, 'previous_source': old_source,
    'verified_source_files': list(source_checks.values()), 'historical_mapping_sources': historical_sources, 'changed_paths_since_run5': subprocess.check_output(
...
Against [run 5](run5-comparison.json), all {len(exact)} failures retain the same identity and normalized cause.
