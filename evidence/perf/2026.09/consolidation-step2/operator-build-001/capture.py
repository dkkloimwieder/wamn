#!/usr/bin/env python3
"""One-use capture of the authorized generator tests and native operator binaries."""
import hashlib
import json
import os
...
expected_head = '52ac90a6400e4b955a90d3f97733c0c5be8a5418'
...
commands.append(('native-operator-build', [
    'cargo', 'build', '--locked', '--offline',
    '-p', 'wamn-ctl', '-p', 'wamn-identity', '-p', 'wamn-host',
    '-p', 'wamn-scenario-worker', '-p', 'wamn-receiving-tui',
    '--bin', 'wamn', '--bin', 'wamn-identity', '--bin', 'wamn-host',
    '--bin', 'wamn-scenario-worker', '--bin', 'wamn-receiving',
    '--message-format=json-render-diagnostics',
]))
requested_binaries = ['wamn', 'wamn-identity', 'wamn-host', 'wamn-scenario-worker', 'wamn-receiving']
...
(evidence / 'status-before.txt').write_bytes(subprocess.check_output(['git', 'status', '--short'], cwd=root))
...
results = []
try:
    with tempfile.TemporaryDirectory(prefix='wamn-operator-build-') as scratch:
        env['TMPDIR'] = scratch
        ...
        for index, (name, argv) in enumerate(commands):
            ...
            compile_failure = process.returncode != 0 and (name == 'native-operator-build' or not summaries or 'could not compile' in err)
            ...
            record = {'name': name, 'argv': argv, 'cwd': str(root), 'exit_code': process.returncode,
                       ...
                       'seconds': round(time.monotonic() - started, 3), 'summaries': counts, 'cases': cases,
            ...
            if process.returncode != 0:
                write('status.json', {'state': 'stopped_on_failure', 'command': name,
                                      'completed': len(results), 'not_run': [item[0] for item in commands[index + 1:]]})
                break
            if name == 'native-operator-build':
                artifacts = {}
                for line in out.splitlines():
                    ...
                    if target['kind'] != ['bin'] or target['name'] not in requested_binaries:
                        continue
                    path = Path(message['executable'])
                    assert path.is_file() and path.is_relative_to(root / 'target/debug')
                    binary_hash = hashlib.sha256()
                    with path.open('rb') as stream:
                        for chunk in iter(lambda: stream.read(1024 * 1024), b''):
                            binary_hash.update(chunk)
                    assert target['name'] not in artifacts
                    artifacts[target['name']] = {
                        'path': str(path), 'sha256': binary_hash.hexdigest(),
                        'bytes': path.stat().st_size, 'mode': oct(stat.S_IMODE(path.stat().st_mode)),
                        'package_id': message['package_id'], 'cargo_fresh': message['fresh'],
                        'target': target, 'profile': message['profile'],
                    }
                assert sorted(artifacts) == sorted(requested_binaries), 'missing emitted requested binary'
    ...
    (evidence / 'status-after.txt').write_bytes(subprocess.check_output(['git', 'status', '--short'], cwd=root))
    complete = len(results) == len(commands) and (evidence / 'artifacts.json').exists()
