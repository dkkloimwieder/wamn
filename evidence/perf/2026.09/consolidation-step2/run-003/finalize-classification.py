...
out=root/'docs/perf/2026.09/consolidation-step2/run-003'
...
sources={
 'writer_scan':excerpt('tests/conformance/tests/state_ownership.rs',892,975),
 'unqualified_target':excerpt('tests/conformance/tests/state_ownership.rs',1535,1550),
...
assert len(value['failures'])==82
assert 'app_move_expanded_static_writer_scan' not in value['classification_counts']
value.pop('manual_review_required')
value['unresolved_classifications']=[]
value['interpretation']='The run failed. All 82 failures remain failures. The 84 explicit skips did not execute their skipped work. No live inputs were armed.'
...
previous=read(out.parent/'run-002/workspace-results.json')
comparison=compare.compare(raw,previous,reducer,causes)
assert len(comparison['absent_reference_failures'])==1
assert not comparison['added_failures']
assert not comparison['changed_causes']
comparison.update(source=source,reference_source=read(out.parent/'run-002/run.json')['source_head'])
...
text=f'''The final app-move workspace run failed on `{source}`. It reports 82 failed tests, 38 failed targets, and 84 explicit skips. It exited 101 after {run['elapsed_seconds']} seconds. All {len(before):,} tracked files outside Beads kept their bytes and modes. HEAD stayed unchanged. [Run](run.json), [source stability](source-stability.json), [full log](workspace.log).
...
The run reports 2,189 passes, which include 84 explicit skips, and six passing doctests. Subtracting known skips gives 2,105 reported passes. Silent early returns can remain undetected, so this is not an exact count of executed work.

All 82 remaining failure identities and causes match Step 1. No new failure remains from the app moves. Two old failing cases and one old skipped case were deleted with native C. Those removals are not passing tests. The baseline comparison also records the earlier renamed app-build input. [Step 1 comparison](step1-comparison.json), [baseline comparison](baseline-comparison.json), [removal records](known-test-removals.json).

The one remaining failure from the second app-move run now passes. The static platform writer scan reads crate directories from Cargo. Authored application SQL keeps its existing application tests and remains in the separate security scan. All 31 focused ownership tests passed before this sweep. [Previous comparison](previous-run-comparison.json), [captured source](reviewed-source-excerpts.json), [focused test output](../state-scope-001/output.log).

This records the completed app-move step against the retained baseline. The workspace still fails, and the skipped live work remains unexecuted in this run. Earlier failed runs retain their original results. The step also retains its separate guest-byte comparisons and live Receiving operator results. No cluster-backed WMS completion claim follows from this sweep.

Every failure and explicit skip follows, with exact retained identifiers.
...
write('classification-summary.json',dict(source=source,exit_code=raw['exit_code'],counts=counts,classification_counts=classes,added_failure_identities_vs_step1=0,removed_failure_identities_vs_step1=2,unchanged_causes_vs_step1=82,explicit_self_skips=84,unresolved_classifications=[],tracked_files=len(before),source_stable=True,log_sha256=run['workspace_log_sha256'],report_sha256=hashlib.sha256((out/'report.md').read_bytes()).hexdigest()))
