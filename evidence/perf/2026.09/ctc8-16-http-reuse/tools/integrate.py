#!/usr/bin/env python3
"""Fast-forward only the approved HTTP change, preserving unrelated local work."""
import argparse
import hashlib
import json
import os
...
PREFIX = "docs/perf/2026.09/ctc8-16-http-reuse/"
...
# The twenty non-evidence paths in bbcdbecd; follow-up commits get no wider scope.
ALLOWED = frozenset((
    "Cargo.lock", "Cargo.toml", "crates/execution/host/src/router_driver.rs",
    "crates/platform/runtime/Cargo.toml",
    "crates/platform/runtime/src/plugins/connection_http.rs",
    "crates/platform/runtime/src/plugins/connection_http/transport.rs",
    "crates/platform/runtime/src/plugins/connection_http/transport/tests.rs",
    "crates/platform/runtime/tests/executor_plugin_no_trap_live.rs",
    "docs/PLAN/PLAN.md", "docs/architecture/native-alignment-ledger.md",
    "docs/architecture/wamn_native_alignment_plan.md", "docs/exe-model.md",
    "docs/operations/build-and-test.md", "services/executor/src/lib.rs",
    "services/host/src/host.rs", "tests/conformance/tests/repo_lint.rs",
    "tests/integration/Cargo.toml", "tests/integration/src/route_authentication_live.rs",
    "tests/integration/src/trusted_http_route.rs", "tools/repo-lint",
))
...
def main():
    ...
    require(all(name in ALLOWED or name.startswith(PREFIX) for name in changed),
            "incoming paths exceed the explicit HTTP allowlist")
