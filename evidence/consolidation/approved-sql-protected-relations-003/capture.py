...
cases = {'protected-relations': ('WAMN_CTL_PG_URL', ['cargo', '+1.98.0', 'test', '--locked', '--offline', '-p', 'wamn-ctl', '--features', 'ops', '--test', 'protected_relations_live', '--', '--ignored', '--nocapture'])}
if len(sys.argv) > 1:
    case = sys.argv[1]
    key, argv = cases[case]
    url = urllib.parse.urlunsplit(('postgresql', 'postgres' + ':' + urllib.parse.quote(os.environ['PGPASSWORD'], safe='') + '@127.0.0.1:' + os.environ['PGPORT'], '/postgres', '', ''))
    env[key] = url
    env['WAMN_UPDATE_PROTECTED_RELATIONS'] = '1'
    pg = dict(version=subprocess.check_output(['psql', '-X', '-Atqc', 'SHOW server_version'], text=True).strip(), configuration_root=os.environ['PG_CLUSTER_CONF_ROOT'], port=os.environ['PGPORT'], login='postgres')
    (out / (case + '-postgres.json')).write_text(json.dumps(pg, indent=2) + '\n')
    # The temporary login matches the established snapshot's object owner.
    password_sql = os.environ['PGPASSWORD'].replace("'", "''")
    subprocess.run(['psql', '-X', '-v', 'ON_ERROR_STOP=1', '-q'], input="DO $$ BEGIN IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname='postgres') THEN CREATE ROLE postgres LOGIN SUPERUSER; ELSE ALTER ROLE postgres LOGIN SUPERUSER; END IF; END $$; ALTER ROLE postgres PASSWORD '" + password_sql + "';", text=True, check=True)
