...
cases = {'publication': ('WAMN_CTL_PG_URL', ['cargo', '+1.98.0', 'test', '--locked', '--offline', '-p', 'wamn-ctl', '--test', 'publish_release_live', '--', '--nocapture', '--test-threads=1']), 'binding': ('WAMN_CONTROL_PORTABLE_PG_URL', ['cargo', '+1.98.0', 'test', '--locked', '--offline', '-p', 'wamn-control-provision', '--test', 'control_portable_store', 'deployment_attestation_rust_binding_holds_on_postgres', '--', '--exact', '--nocapture'])}
if len(sys.argv) > 1:
    case = sys.argv[1]
    key, argv = cases[case]
