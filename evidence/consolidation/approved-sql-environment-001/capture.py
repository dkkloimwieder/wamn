...
cases = {'tenant-and-instance': ('WAMN_CTL_PG_URL', ['cargo', '+1.98.0', 'test', '--locked', '--offline', '-p', 'wamn-ctl', '--lib', 'provision_project_env::tests::tenant_projection_and_instance_claim_hold_on_postgres', '--', '--exact', '--nocapture']), 'component-instance': ('WAMN_CTL_PG_URL', ['cargo', '+1.98.0', 'test', '--locked', '--offline', '-p', 'wamn-ctl', '--lib', 'push_component::tests::the_environment_instance_decides_which_run_owns_a_component_fact', '--', '--exact', '--nocapture'])}
if len(sys.argv) > 1:
    case = sys.argv[1]
    key, argv = cases[case]
...
for case in cases:
    ...
    records.append(dict(case=case, exit=result, temporary_cluster_removed=cleaned, source_unchanged=unchanged))
    (out / 'result.json').write_text(json.dumps(records, indent=2) + '\n')
    if result or not unchanged: break
raise SystemExit(any(r['exit'] or not r['source_unchanged'] or r['temporary_cluster_removed'] is False for r in records))
