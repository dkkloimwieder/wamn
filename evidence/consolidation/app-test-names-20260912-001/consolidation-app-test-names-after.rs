...
fn inspect(value:Value)->Value {match serde_json::from_value::<Inputs>(value) {Ok(input)=>json!({"accepted":input.test_id}),Err(error)=>json!({"error":error.to_string()})}}
...
let mut diagnostics=Vec::new();for stdout in ["", "run completed: wrong\n", "run completed: one\nrun completed: two\n"] {let results=stdout.lines().filter(|line|line.starts_with("run completed:")).collect::<Vec<_>>();diagnostics.push(format!("wamn dev returned the wrong product receipt: {results:?}; stdout={stdout:?}"));}
