...
compile_record = out.parent / 'run-state-temp-fix-20260912-001' / 'compile.json'
...
if '--inside' in sys.argv:
    ...
    prepare = "CREATE ROLE postgres LOGIN SUPERUSER CREATEDB CREATEROLE INHERIT REPLICATION BYPASSRLS; SELECT rolcanlogin AND rolsuper AND rolcreatedb AND rolcreaterole AND rolinherit AND rolreplication AND rolbypassrls FROM pg_roles WHERE rolname='postgres';"
    code, text = capture(['psql', '-X', '-v', 'ON_ERROR_STOP=1', '-Atqc', prepare], 'prepare-postgres-role', os.environ.copy(), private)
    assert code == 0 and text.strip() == 't', 'the catalog requires the postgres administrative role'
    ...
    save('observations.json', {'test_exit_code':test_code, 'reported_counts':counts, 'public_temporary_revoked_before_test':True, 'executor_privilege_check_exit_code':check_code, 'executor_temporary_actual':privilege.strip() if check_code == 0 else None, 'executor_temporary_absent_after_test':privilege_absent})
...
result = {'source':source,'compile_record':str(compile_record),'rebuild':False,'test_artifact':compiled['test_artifacts'][0],'exit_code':exit_code,'cleanup':cleanup,'source_unchanged':unchanged}
