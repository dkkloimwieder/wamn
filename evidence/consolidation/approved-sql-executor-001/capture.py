...
cases = {
    'executor-current-user': ('WAMN_PRODUCTION_CLAIM_PG_URL', ['cargo', '+1.98.0', 'test', '--locked', '--offline', '-p', 'wamn-runtime', '--lib', 'executor_authority_uses_current_user_for_each_operation', '--', '--ignored', '--nocapture']),
    'executor-surface': ('WAMN_EXEC_PLATFORM_PG_URL', ['cargo', '+1.98.0', 'test', '--locked', '--offline', '-p', 'wamn-runtime', '--test', 'executor_platform_surface_live', '--', '--ignored', '--nocapture']),
}
if len(sys.argv) > 1:
    case = sys.argv[1]
    key, argv = cases[case]
