    ...
    fn run(context: NodeContext, input: String) -> Result<Emission, NodeError> {
        ...
        let key = input
            ...
            .ok_or_else(|| {
                invalid_input(
                    "missing_key",
                    "key is required: the caller supplies a deterministic object key, and this \
                     node never generates one",
                )
            })?;
        let body = input
            .get("body")
            .and_then(serde_json::Value::as_str)
            .ok_or_else(|| invalid_input("missing_body", "body must be a string"))?;
