//! Custom-node fixture importing only the typed HTTP connection capability.
...
impl exports::wamn::connection_http_custom_fixture::node::Guest for Component {
    fn run(input: u32) -> u32 {
        let request = wamn::connection::http::Request {
            requirement: "custom-erp".to_string(),
            method: "POST".to_string(),
            path_and_query: "/receipts?source=custom".to_string(),
            headers: Vec::new(),
            body: None,
