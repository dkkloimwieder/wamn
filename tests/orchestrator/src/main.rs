...
use wamn_integration_tests::{
    ...
    readerbench, rc, record_history_retention, retention,
};
...
enum Command {
    ...
    /// Test that the real prune-record-history verb removes only expired record history entries as the audit retention credential.
    RecordHistoryRetention(record_history_retention::RecordHistoryRetentionArgs),
...
async fn async_main() -> anyhow::Result<()> {
    ...
    let result = match cli.command {
        ...
        Command::RecordHistoryRetention(args) => record_history_retention::run(args).await,
