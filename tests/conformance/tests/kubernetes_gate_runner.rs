...
case "$command" in
  get)
    ...
    if [[ "$kind" == job ]]; then
      ...
      printf '{"items":[{"metadata":{"name":"%s-pod","uid":"pod-%s","creationTimestamp":"9998-01-01T00:00:00Z"},"spec":{"nodeName":"wamn-worker","initContainers":[{"name":"ready"}],"containers":[{"name":"%s","image":"%s"}]},"status":{"startTime":%s,"phase":"%s","initContainerStatuses":%s,"containerStatuses":[{"name":"%s","imageID":"docker-pullable://wamn@sha256:abc","state":{"terminated":{"exitCode":%s,"finishedAt":"9998-01-01T00:00:02Z"}}}]}}]}\n' \
        "$name" "$name" "$name" "$image" "$started" "$phase" "$init" "$name" "$exit_code"
    fi
    ;;
  delete)
    : >"$FAKE_DELETED"
