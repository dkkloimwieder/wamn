...
const PLATFORM_PLAN: &str = include_str!("../../../docs/archive/platform-plan.md");
const AUTHORITATIVE_LEDGER_STATEMENT: &str = "`docs/archive/platform/wash-runtime-fork.md` is the authoritative branch, revision, and carried-policy ledger";

#[test]
fn d23_current_state_names_the_current_fork() {
    ...
    let d23 = d23_rows[0];
    assert!(
        d23.contains("`wamn/2.7.0`"),
        "D23 current state must name the v2.7.0 fork branch"
    );
    for superseded in ["`wamn/2.5.2`", "`wamn/2.6.0`", "`wamn/2.6.1`"] {
        assert!(
