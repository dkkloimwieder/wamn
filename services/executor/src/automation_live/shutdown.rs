//! Real process signals while production-admitted work executes a native guest.
...
pub(super) async fn run(
    ...
    probe_tasks.spawn(std::future::pending::<anyhow::Result<()>>());
    ...
    let serving = super::super::supervise_queue(
        queue,
        stop,
        &mut probe_tasks,
        &probes,
    ...
    timeout(Duration::from_secs(10), async {
        loop {
            tokio::select! {
                result = &mut serving => panic!("executor stopped before guest entry: {result:?}"),
    ...
    let error = timeout(Duration::from_secs(8), &mut serving)
        .await
        .expect("bounded process-signal drain")
        .expect_err("active spinning guest exceeds drain budget");
    assert!(
        format!("{error:#}").contains("shutdown budget"),
        "{error:#}"
    );
    assert!(sent.elapsed() < Duration::from_secs(8));
...
fn isolated(signal: &str, name: &str) {
    const CHILD: &str = "WAMN_EXECUTOR_SHUTDOWN_CHILD";
    if std::env::var(CHILD).as_deref() != Ok(signal) {
        let output = std::process::Command::new(std::env::current_exe().unwrap())
            .args([
                "--exact",
                &format!("automation_live::shutdown::{name}"),
                "--nocapture",
            ])
            .env(CHILD, signal)
            .output()
            .expect("isolated signal test");
        assert!(
    ...
    let watchdog = std::thread::spawn(move || {
        if finished.recv_timeout(Duration::from_secs(50))
    ...
    let runtime = tokio::runtime::Builder::new_multi_thread()
        ...
        .unwrap();
    runtime
        .block_on(super::run_automation(Some(signal)))
        .expect("real queued shutdown");
    drop(runtime);
    done.send(()).unwrap();
...
#[test]
fn sigterm_during_admitted_guest_preserves_recovery_and_revokes_authority() {
    isolated(
        "-TERM",
        "sigterm_during_admitted_guest_preserves_recovery_and_revokes_authority",
    );
}

#[test]
fn sigint_during_admitted_guest_preserves_recovery_and_revokes_authority() {
    isolated(
        "-INT",
        "sigint_during_admitted_guest_preserves_recovery_and_revokes_authority",
    );
}
