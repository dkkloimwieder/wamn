...
async fn authoring_command(
    ...
    let presented = bearer(&request).map(str::to_owned);
...
mod tests {
    ...
    #[test]
    fn the_surface_serves_exactly_the_two_reserved_routes() {
        let source = include_str!("management.rs");
        let router = between(source, "async fn route(", "async fn login(");
        assert!(router.contains(r#"(&Method::POST, "/login")"#));
