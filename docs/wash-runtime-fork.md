...
| `0aee2546` "feat(wash-runtime): inject outbound trace context (wamn-8zht.4)" | Functional. On v2.6.0, the `HttpServer` P2 and P3 host surfaces inject the current W3C trace context at their common pre-transport seams, before the HTTP/gRPC/custom-transport branch. P2 initially carries caller context so custom transports preserve continuity; built-in P2 HTTP and gRPC transports re-inject client-span context for correct downstream parenting. P3 creates its client span before the common seam, so P3 HTTP, gRPC, and custom transports receive client-span context. This is host-enforced for every admitted outbound request dispatched through those surfaces; `DefaultOutgoingHandler` is not universal on v2.6.0. The global no-op propagator injects nothing when observability is off. | upstream provides equivalent host-enforced P2/P3 trace-context injection across HTTP, gRPC, and custom transports, including client-span parenting — delete the commit |
| `89d24ebc` "fix(runtime): deny raw TCP without opt-in (wamn-8zht.5)" | Security. On v2.6.0, `wasi:sockets` remains linked independently of the `wasi:http` egress allowlist, so `build_ctx_from_template` denies `TcpConnect` unless the active component opts in. `wamn.allow-raw-sockets` config takes precedence over `WAMN_ALLOW_RAW_SOCKETS`; absent or unparseable values deny. Literal and loopback TCP are denied without opt-in, and the first denial warns once per component under `wamn::sockets`. `allowIpNameLookup` is an independent primitive and does not satisfy this policy. The new upstream P3 loopback-gateway fixture explicitly declares the raw-socket capability it consumes. UDP behavior remains upstream in this commit and is owned separately by wamn-8zht.6. | upstream gates socket linking on `host_interfaces`, or consults an egress policy for `TcpConnect` — delete the commit |
| `981fdc5` "wamn: memory-limiter accessors + wamn.api.requests host HTTP counter" | Observability (9.8, wamn-jn6). On branch `wamn/jn6-metrics` (off `eef76cd`; integrated onto the carried line on push). Two host-integration seams the wamn host cannot reach from outside the runtime: (1) `WamnStoreLimiter` (`engine/ctx.rs`) gains read-only accessors — `component_id` / `budget_bytes` / `high_water_bytes` / `denied_total` — so a host observable instrument bridges per-store linear-memory state to `wamn.memory.*` metrics instead of re-parsing the `wamn::memory` denial logs (the limiter fields stay private; only getters are added — no behavior change). (2) `record_response_status` (`host/http.rs`, the single per-request choke point in `handle_http_request`, called on the normal invoke + the routing-failure early returns) increments a `wamn.api.requests` `u64` counter on the global meter, labelled `status_class` (2xx/4xx/5xx/…). A no-op until `initialize_observability` installs a provider — byte-identical to upstream then — and symmetric to the existing `fuel.consumption` histogram the fork already emits. The host owns the HTTP server, so this is the only seam that sees every generated-API request (`ProxyPre` benches bypass it). | upstream exposes limiter introspection accessors AND an inbound HTTP request-count metric — delete this commit |
| `11bef6ee` "fix(runtime): deny raw UDP without opt-in (wamn-8zht.6)" | Security. On v2.6.0, `UdpConnect` and `UdpOutgoingDatagram` share the raw-socket opt-in resolved as component config, then `WAMN_ALLOW_RAW_SOCKETS`, then DENY; unparseable values fail closed. `UdpBind` is service-loopback-only and denied for non-service components, and raw-egress opt-in never widens bind authority. P2 unconnected sends explicitly consult `UdpOutgoingDatagram`. P3 unconnected sends consult `UdpOutgoingDatagram` before socket-state inspection, then separately consult `UdpBind` before the one-time implicit bind, preserving both ordered denial points. Warn-once visibility now covers TCP and UDP raw egress. `allowIpNameLookup` remains independent. | upstream gates socket linking on `host_interfaces`, or consults an egress policy for UDP connect/datagram operations — delete the commit |
