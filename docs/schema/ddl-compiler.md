...
> capture is CDC via logical decoding (`docs/events/event-plane-jetstream.md`): the
...
> (b687d45) and the C2 trigger-overhead campaign in docs/results/ceilings.md. What
...
- Catalog model (the input): `docs/schema/catalog-model.md`, `crates/schema/model`.
- Tenant shape: `deploy/sql/postgres-init.sql`, `docs/data-path/security-db-path.md`.
