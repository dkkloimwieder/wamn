...
It records the subsequent direct and nested HTTP passes. Final integrated and deployed validation remains pending.
...
The manual application cache, linker lifecycle, stores, and epoch conversion are removed from the integration.
Row 4 records that mechanism change and keeps warm reuse refused.
The integrated correctness gates and deployed proofs remain pending under `wamn-0ct2.2`.
