...
- **Leakage through the tree itself.** This document lives in the repository the
  ...
  runner now removes `evidence/experiments/` from the run worktree and commits the
...
- **The rubric is in the agent's own task directory.** `WAMN_PILOT_TASK_DIR`
  ...
  run 001 twenty-four. Removing `evidence/experiments/` from the worktree, the fix
...
- **The instrument was readable too.** Both runs of series 020 read
  ...
  `evidence/experiments`, every `tools/agent-pilot-*` file, and the `[AGENT-PILOT]`
