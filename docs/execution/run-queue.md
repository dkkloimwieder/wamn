...
> (`docs/events/event-plane-jetstream.md`): the reader publishes onto JetStream and
...
- **`queuebench`** — the gate of record (pure host-side `tokio_postgres` claimers
  ...
  **janitor** sweep, the **NATS-core
  ...
  `docs/events/event-plane-jetstream.md` §10) is a measurement *campaign*, deliberately
  **not** part of `--mode all`: open-loop producers + closed-loop claimers drive
  ...
  burst/recovery profile — curves + CSVs published to `docs/results/ceilings.md`, with
