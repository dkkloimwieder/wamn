...
an explicit owner deferral; `.1.14` supplies exactly such a development
decision plus granular production deferrals.
...
`.1.15`, and compatibility semantics belong to `.1.16`.
...
DR/Secret/etcd authority (`.1.18/.1.15/.1.17`), dispatcher progress metrics
...
| **Edge/on-prem** | No recovery profile is offered at this stage. | Offline authority, backup, upgrade, and operator behavior remain unsupported rather than guessed. | Cardinality/deployment `.1.12`, isolation `.1.15`, compatibility `.1.16`, and recovery `.1.18`–`.24` as applicable. |
