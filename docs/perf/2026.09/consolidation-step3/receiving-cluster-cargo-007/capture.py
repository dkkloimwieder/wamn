...
out=root/'docs/perf/2026.09/consolidation-step3/receiving-cluster-cargo-007'
live=root/'docs/perf/2026.09/consolidation-step3/receiving-cluster-live-007'
expected='ed16ee43f6486ee6f4961d64d6f1db99b23731f3'
...
ordinary=run(common+['route_authentication_live::cluster::operator_recovery::tests','--','--nocapture'],out/'ordinary',{'RUSTC_WRAPPER':'','CARGO_BUILD_JOBS':'2'})
if ordinary==0:
    ...
    code=ordinary
    write(out/'record.json',{'source_commit':expected,'exit_code':code,'full_default_executed':False,'reason':'ordinary operator recovery tests failed','finished_utc':now()})
after=snapshot()
