#!/usr/bin/env python3
"""Complete the source review for the third stage 3 test run."""
...
for key, row in new_index.items():
    ...
    for field in ['reviewed_explanation', 'execution_scope']:
        if old.get(field):
            row[field] = old[field]
...
restricted_run = run.parent / 'run-002'
restricted_raw = read(restricted_run / 'reduction-001/workspace-results.json')
restricted_value = read(restricted_run / 'classification-002/classified-failures.json')
restricted_source = restricted_value['source']
restricted_comparison = compare_module.compare(raw, restricted_raw, reducer, causes)
restricted_comparison.update(source=source, reference_source=restricted_source,
    commands_equal=read(run / 'command.json') == read(restricted_run / 'command.json'))
restricted_index = {identity(row): row for row in restricted_value['failures']}
recovered = []
for key in restricted_comparison['absent_reference_failures']:
    old = restricted_index[tuple(key)]
    assert old['classification'] == 'socket_permission_refusal'
    result = case_result(key[2])
    assert result['status'] == 'ok'
    recovered.append({'identity': key, 'previous_cause': old['cause'], 'current_result': result,
        'disposition': 'The case reports a passing result in the host execution context.'})
source_changes = subprocess.check_output(['git', 'diff', '--name-only', restricted_source, source], cwd=root, text=True).splitlines()
assert source_changes == ['apps/wamn_wms/README.md']

moves = read(run / 'app-test-moves-prepared.json')
assert moves['source'] == restricted_source and moves['previous_source'] == old_source
moves['preparation_source'] = moves['source']
moves['source'] = source
for entry in list(moves['source_records']):
    original_bytes = source_bytes(entry['source'], entry['path'])
    assert hashlib.sha256(original_bytes).hexdigest() == entry['sha256']
    if entry['source'] == restricted_source:
        current = source_bytes(source, entry['path'])
        assert current == original_bytes and hashlib.sha256(current).hexdigest() == before[entry['path']]['sha256']
...
write('run2-comparison.json', restricted_comparison)
write('socket-refusals-now-passed.json', {'source_changes_since_run2': source_changes, 'cases': recovered})
...
summary = {'source': source, 'exit_code': raw['exit_code'], 'elapsed_seconds': run_record['elapsed_seconds'],
    ...
    'baseline_moved_failures': len(baseline_moves), 'socket_refusals_now_passed': len(recovered)}
...
This capture used the host execution context after run 2 denied local socket operations.
All {len(recovered)} [cases denied sockets in run 2](socket-refusals-now-passed.json) now report passing results.
Only the WMS README changed between those source revisions. The source diff is recorded with those passing rows.
The chart case again reaches its previous Kubernetes discovery refusal. [Run 2 comparison](run2-comparison.json).

Against [run 1](run1-comparison.json), all {len(exact)} remaining failures retain the same identity and normalized cause.
No failure identity was added, and no remaining cause changed.
The three corrected cases now report passing results. [Their exact names and passing rows](changed-failures.json) remain listed.
...
Every failure is classified, with no unresolved parser entry or cause review. This failed result supplies no application or wave completion claim.
