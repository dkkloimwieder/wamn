#!/usr/bin/env python3
"""Reduce the stage 3 result with the unchanged retained reducer APIs."""
import argparse
...
paths = {
    'retained_runner': month / 'generated-tui-integration/tools/workspace.py',
    'retained_comparator': month / 'receiving-postcommit/tools/compare_integrated_sweep.py',
    'retained_reducer': month / 'wasmcloud-2-9-cutover/workspace-integration-preparation-001/classify.py',
    'retained_cause_normalizer': month / 'ctc8-20-pat-service/main-landing-001/compare.py',
    'reducer_legacy_baseline': month / 'wasmcloud-2-9-cutover/validation-001/workspace-results.json',
}
references = {
    'baseline': month / 'consolidation-baseline/run-001',
    'step1': month / 'consolidation-step1/run-001',
    'step2': month / 'consolidation-step2/run-003',
}
for name, directory in references.items():
    for suffix, filename in [('reduced', 'workspace-results.json'),
                             ('classified', 'classified-failures.json'),
                             ('command', 'command.json'), ('log', 'workspace.log')]:
        paths[name + '_' + suffix] = directory / filename
...
def main():
    ...
    comparator = load(paths['retained_comparator'], 'stage3_retained_comparator')
    reducer = comparator.load(paths['retained_reducer'], 'stage3_retained_reducer')
    causes = comparator.load(paths['retained_cause_normalizer'], 'stage3_retained_causes')
    ...
    reference_results = {name: read(paths[name + '_reduced']) for name in references}
    ...
    for name, comparison in comparisons.items():
        ...
        comparison['reference_source'] = (references[name] / 'source.txt').read_text().strip()
        comparison['commands_equal'] = read(evidence / 'command.json') == read(paths[name + '_command'])
        comparison['counts'] = current['counts']
        comparison['reference_counts'] = reference_results[name]['counts']
        def skip_names(result):
            targets = {row['running_log_line']: row for row in result['test_targets']}
            return Counter((row['target_description'],
                re.sub(r'-[0-9a-f]+$', '', Path(targets[row['target_running_log_line']]['executable']).name),
                row['name']) for row in result['explicit_self_skips']['entries'])
        old_skips, new_skips = skip_names(reference_results[name]), skip_names(current)
    ...
    old_classified = {tuple(entry[key] for key in ['package', 'cargo_target', 'name']): entry
                      for entry in read(paths['step2_classified'])['failures']}
    exact = {tuple(identity) for identity in comparisons['step2']['exact_identity_and_cause_matches']}
    classified = []
    review = []
    for failure in current['failures']:
        ...
        entry.update(classification=prior['classification'] if identity in exact else 'requires_current_cause_review',
                     ...
                     step2_comparison='exact normalized diagnostic match' if identity in exact else 'review required')
        if prior:
            entry['step2_classification'] = prior['classification']
            entry['step2_cause'] = prior['cause']
    ...
    known_removals = read(references['step2'] / 'known-test-removals.json')
    disposition = {}
    for name, comparison in comparisons.items():
        rows = []
    ...
    write(out, 'classified-failures-draft.json', {
        ...
        'interpretation': 'Draft classification. Changed, new, and absent cases need diagnostic or source review. Removed cases are not passing cases.',
    })
    ...
    write(out, 'classification-inputs.json', [record(paths[name]) for name in
        ['retained_comparator', 'retained_reducer', 'retained_cause_normalizer', 'reducer_legacy_baseline',
         'baseline_reduced', 'step1_reduced', 'step2_reduced',
         'baseline_classified', 'step1_classified', 'step2_classified']]
        + [record(references['step2'] / 'known-test-removals.json'), record(Path(__file__))]
