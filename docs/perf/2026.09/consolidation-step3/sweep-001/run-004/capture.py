#!/usr/bin/env python3
"""Capture source stability around the retained stage 3 workspace command."""
import gzip
import hashlib
import json
import os
...
expected = sys.argv[1]
relative = Path('docs/perf/2026.09/consolidation-step3/sweep-001') / (sys.argv[2] if len(sys.argv) > 2 else 'run-001')
evidence = root / relative
assert not evidence.exists(), 'Use a new result directory. Prior results stay unchanged.'
head = subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=root, text=True).strip()
assert head == expected
def tracked_paths():
    return [os.fsdecode(path) for path in subprocess.check_output(
...
def snapshot(selected):
    ...
    for relative_path in selected:
        ...
        if not path.exists() and not path.is_symlink():
            result[relative_path] = {'kind': 'missing'}
            continue
        info = path.lstat()
...
paths_after = tracked_paths()
after = snapshot(paths_after)
...
write('capture-command.json', sys.orig_argv)
