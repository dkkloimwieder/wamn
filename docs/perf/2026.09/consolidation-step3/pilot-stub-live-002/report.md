...
The original [secret review](secret-review.json) compared seven known credentials and common credential shapes before cleanup. It found zero matches in the reviewed files. No new comparison used the removed credential files. [raw-log-restriction.json](raw-log-restriction.json) preserves the earlier rejection because that authorization covered run 901s only. The later [approval record](raw-log-publication.json) records the user response, named payload and destination. It authorizes this exact run 902s log. The copy preserves all 41,205 bytes and its original mode. The run 903s raw log remains excluded. This result does not establish a successful pilot or application run.
