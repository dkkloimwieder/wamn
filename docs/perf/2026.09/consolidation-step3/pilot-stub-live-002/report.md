...
The original [secret review](secret-review.json) compared seven known credentials and common credential shapes before cleanup. It found zero matches in the reviewed files. No new comparison used the removed credential files. [raw-log-restriction.json](raw-log-restriction.json) preserves the earlier rejection because that authorization covered run 901s only.
