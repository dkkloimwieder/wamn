...
try:
    ...
    ready_command = ['docker', 'exec', container, 'pg_isready', '-h', '127.0.0.1', '-U', 'postgres', '-t', '1']
    ready_started = time.monotonic()
    deadline = ready_started + 30
    attempts = 0
    with (evidence / 'ready.log').open('w') as output:
        while True:
            remaining = deadline - time.monotonic()
            if remaining <= 0:
                raise RuntimeError('PostgreSQL did not accept TCP connections within 30 seconds')
            attempts += 1
            print(f'attempt={attempts}', file=output, flush=True)
            ready = subprocess.run(ready_command, stdout=output, stderr=subprocess.STDOUT,
                                   timeout=remaining)
            if ready.returncode == 0:
                break
            if ready.returncode not in (1, 2):
                raise RuntimeError(f'PostgreSQL readiness command exited {ready.returncode}')
            time.sleep(min(0.2, max(0, deadline - time.monotonic())))
    commands.append({'name': 'ready', 'argv': ready_command, 'exit_code': 0,
                     'attempts': attempts, 'limit_seconds': 30,
    ...
    with (evidence / 'container.log').open('w') as output:
        subprocess.run(['docker', 'logs', '--timestamps', container],
