...
**Status:** draft 0.2 for external review · 2026-09-09 · review amendments applied  
**Scope:** executable tests for platform guarantees and application business rules; Rust-only initial delivery.  
**Relationship:** refines the existing deterministic-testing proposal and application-testing procedure. This is a proposed delivery contract, not an implementation-status report. Existing product contracts remain authoritative. [1–4]
...
**Kani is out of scope for this delivery**, applying the owner’s incremental-adoption steer and explicit request to hold off. Approval records this deferral; the earlier reviewer-proposed numeric-bug trigger does not open Kani work under this spec. Reopening requires an explicit owner decision. No Kani dependencies, harnesses, or toolchain work. Coverage-guided fuzzing, Bolero, and production invariant tripwires remain deferred. [1]
...
**Proposed clarification to [1]:** D2 may replace redundant sequential crash scenarios, but a simulated crash does not automatically retire real process-kill, connection-loss, or contention tests. Retire a test only when its actual guarantee is covered elsewhere.
...
Respect existing platform-wave fences. On approval, reconcile the older proposal and authoring procedure with these decisions; Beads and executed receipts own completion. Do not maintain competing plans.

**Approval requested:** the layered approach, Receiving first, the fresh-install-only and eventual-outcome overlay slice, and application-owned invariants. Record Kani’s deferral as the owner’s incremental-adoption direction applied to this delivery, with no automatic reopening under the earlier trigger. Exact helper APIs and per-suite budgets are selected in the implementing bead, not frozen here.

## Basis

[1] `docs/poc/deterministic-testing-spec.md`, §0 and Parts A–E: D1–D8, replay, invariants. This draft applies the owner’s current Kani deferral and proposes the crash-test retirement clarification; approval records the changes.
