...
lane = Path('/home/kaalin/dev/wamn')
evidence = Path('/home/kaalin/dev/wamn/docs/perf/2026.09/native-b-adoption') / sys.argv[1]
command = sys.argv[2:]
evidence.mkdir(parents=True, exist_ok=False)
(evidence / 'capture-run.py.txt').write_bytes(Path(__file__).read_bytes())
def git(*args):
...
for line in (evidence/'output.log').read_text().splitlines():
    if line.startswith(('test result:', 'error:', 'authenticated-native', '  Executable')) or 'Finished `' in line:
        print(line)
