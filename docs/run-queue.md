...
This issue ships the **walking skeleton** of the D3 hybrid: the SKIP LOCKED queue,
the write-ahead / fast path, single-owner leases + reclaim, the janitor, and the
reconciliation cadence, proven by `queuebench`. It deliberately does **not** ship
(tracked as follow-ups):

| Deferred | Where |
|---|---|
| Multi-replica **per-partition ownership** for `partitioned(key)` | 5.14 follow-up (`partition_key` reserved) |
...
- **`cargo test -p wamn-run-queue`** — the pure decisions + SQL shape: claim
  eligibility (Ready/Leased/Parked), `plan_claim` ordering + limit, lease liveness
  + renewal, janitor orphan-detection, reconciliation cadence, the SQL builders'
...
- **live-apply** (`WAMN_RUN_QUEUE_PG_URL`) — applies `run-state.sql` +
  ...
  sweep, tenant RLS isolation, and the FK cascade.
- **`queuebench`** — the gate of record (pure host-side `tokio_postgres` claimers
  ...
  reclaim** (crash-safe failover), the **janitor** sweep, and the **NATS-core
  doorbell** async-warm latency (p50 < 25 ms / p99 < 100 ms). Runs co-located with
  Postgres, no CPU limit (the S2 CFS lesson), locally and in-cluster.
