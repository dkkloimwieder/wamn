...
5. Workflow feature: the workflow contract (start, park, release, list), triggers (event, schedule, manual call), platform nodes, approvals. Only when a real workflow is needed.
   Built by Beads `wamn-upl3` ([workflow feature](workflow-feature.md)). `wamn-workflow` exposes `Workflows` with `PostgresWorkflows`, a `wamn.json` workflow declaration registers any wiring on an application event, the JSONata node is a palette node, and the WMS label graph is the workflow `movement_label`. Approvals, schedule triggers, and queued event runs stay open.
