...
- A form whose plan binds a revision reads the record when it opens and sends the revision of that read, never one read at submit. A delete sends the key and the revision of the record that the page displayed, and reads nothing. A revision that states `revision_of` comes from the row that the operator chose for that input. Any other revision with no binding is a prop.
