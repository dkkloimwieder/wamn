...
Epic 16, Beads `wamn-rst8`, made every generated read an HTTP GET with cache headers and ETags, and gave `web/runtime` a read store. BUILT on 2026-09-25, and the epic waits for the owner review. Measured by hand in a browser against fresh local stacks at scale 1000 on 2026-09-25. In Receiving, a list read again after its `max-age` answered 304 with 221 bytes instead of 36 KB. After a supplier change, the order list answered 200 with the new revision, and the unchanged supplier list answered 304. After a reload, an unchanged list answered 304, and the page read the browser copy. WMS showed the same after a move, and a pallet get opened twice answered 304. Three findings stay open. `wamn-v1u0` holds the gets that declare no revision field, so they never answer 304. Each WMS pallet table read therefore sends 100 full location gets. `wamn-hw3n` holds the WMS dev loop, which refuses the label wiring. `wamn-n9yj` holds the Dockerfile guest stage, which no per-issue check builds, so the materializer did not compile from `wamn-rst8.4` until `wamn-32kj`.
