...
2. **Authored labels and descriptions** in the manifest, contract, and IR. Every screen today reads `purchase order line id`, derived from a field path. Epic 6, Beads `wamn-c2y5`. The owner accepted its spec on 2026-09-22 with one ruling: a generated component renders no heading, and the operation label is an exported constant that the page places.
3. **Screen population:** selectors fed by a list, table row to form. Multiple outer items per submit. The demo made the operator paste identifiers by hand, which is the largest ergonomic cost measured. The owner ruling of 2026-09-22 puts `wamn-z5vd` and `wamn-s3kd` here, because both are form input rules.
