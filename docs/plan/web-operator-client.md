...
- A form whose plan binds a revision reads the record when it opens and sends the revision of that read, never one read at submit. A delete sends the key and the revision of the record that the page displayed, and reads nothing. A revision that names an input comes from the row that the operator chose for that input. No page supplies a revision: generation refuses a form whose revision has no binding and names no input (`unsupplied_revision`).
...
- `web/demo` was the disposable page that ran the Receiving or WMS components against a local stack, deleted in `wamn-defe`. It resolved the runtime, the generated client and the four framework packages by alias, because a generated module sits outside the package that installs them. Its Vite proxy gave the browser one origin: the release routes by a `Host` header, and the issuer signs its own certificate.
...
4. **Cookie session and CSRF** in the identity service. Epic 11, Beads `wamn-by1z`. CLOSED on 2026-09-23, and the owner review accepted it. The login, renew and logout endpoints take a `carrier`, and the cookie carrier sets the session token in an HttpOnly, Secure, SameSite=Strict cookie. The token carries the SHA-256 of a readable CSRF cookie, and the router requires the matching `x-wamn-csrf` header on every cookie request. `wamn-glgg` exempts read routes once the route kind reaches the serving manifest. The browser holds no token and writes nothing to web storage. Measured in a browser against a fresh local stack at one thousand purchase orders on 2026-09-23. The demo signed in by cookie, survived two reloads by renewal, and submitted a supplier change. After sign out the old cookies refused with 401, and the cookie jar was empty. A router test sends a real signed cookie: with its CSRF header it is admitted, and without it it refuses with 401.
...
Epic 16, Beads `wamn-rst8`, made every generated read an HTTP GET with cache headers and ETags, and gave `web/runtime` a read store. CLOSED on 2026-09-25 on the workspace test run, by owner ruling. The cluster stage route is pending. Measured by hand in a browser against fresh local stacks at scale 1000 on 2026-09-25. In Receiving, a list read again after its `max-age` answered 304 with 221 bytes instead of 36 KB. After a supplier change, the order list answered 200 with the new revision, and the unchanged supplier list answered 304. After a reload, an unchanged list answered 304, and the page read the browser copy. WMS showed the same after a move, and a pallet get opened twice answered 304. A get then declared no revision field unless its author wrote one, so most gets never answered 304. `wamn-v1u0` closed that: a get now takes its model's revision column, and the client index names each model that has none. Three findings stay open. `wamn-0378` holds two build tool tests that `wamn-rie7` and `wamn-6fkw` broke. `wamn-hw3n` holds the WMS dev loop, which refuses the label wiring. `wamn-n9yj` holds the Dockerfile guest stage, which no per-issue check builds, so the materializer did not compile from `wamn-rst8.4` until `wamn-32kj`.
...
Epic 20, Beads `wamn-xyxj`, built this item in kind and rendered it for Google Cloud, as [web deployment](web-deployment.md) records. CLOSED on 2026-09-25. The owner ruled one public host for every application and no shared cache for authenticated reads, and kept the list values. `wamn web upload` builds a client and writes it to a bucket path named by its release, and each object carries its Cache-Control: `assets/` is `public, max-age=31536000, immutable`, and `index.html` is `no-cache`. The nginx edge in `deploy/platform/edge` serves that path, removes `/api` on the way to the route ingress, and sends `/password` to identity over verified HTTPS. Measured in a browser through the kind edge on 2026-09-25: a cookie sign-in, a purchase order list that answered 304 after its `max-age`, a purchase order get that answered 304, and a supplier change, with the session renewed after a reload. The same chart renders the Google Cloud load balancer from one values file that names the project, the domain and the issuer, and only a rendering checks that part. `wamn-ghx2` holds the first real deployment, which waits for those three names. The workspace test log at `tests/sweeps/wamn-xyxj-05d277acd.log` has 2519 passed and 40 failed. Six of the failures came from the epic and are fixed, and the others are tests without their inputs. The kind edge case passed on its first run on the closeout commit, in 19 minutes with an image build, while a second cluster ran on the machine. Its four earlier failures during `wamn-xyxj.1` were a stale fixture check, a node port that the test called before kube-proxy served it, a short invitation secret, and machine load; the test fixes the first three.
