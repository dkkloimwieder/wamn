...
1. **The defects the demo found.** Epic 5, Beads `wamn-iq82`. CLOSED OUT on 2026-09-22, and the epic waits for the owner review. `wamn-uuo6`: an `int64` is a string and the wire wants a number. `wamn-v8ku`: a refusal marks no field. `wamn-m9qt`: initial values cannot reach a nested member. `wamn-wrlp`: a detail demands a request identity it discards. `wamn-br5r`: a revision reads as 0. The first one blocks a whole operation. The integer ruling of 2026-09-22 governs the fix. An application integer is `int32` by default, and an `int64` is opt-in with a reason. A Postgres internal never enters an application contract. A real `int64` stays a string on the wire and opaque in the browser.
