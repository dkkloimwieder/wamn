...
- `web/demo` is the disposable page that runs the Receiving or WMS components against a local stack. It resolves the runtime, the generated client and the four framework packages by alias, because a generated module sits outside the package that installs them. Its Vite proxy gives the browser one origin: the release routes by a `Host` header, and the issuer signs its own certificate.
