...
Issue `wamn-hxow` measured that premise. No generator writes a component's world: each application writes its world by hand, so a component without SQL already imports no `wamn:postgres`. What refused a command without SQL was the manifest rule that every command declares `idempotent_by` as claim, state or inherited, and all three need SQL or a base dependency. The owner ruled a fourth explicit shape, `"idempotent_by": "stateless"`. Generate and publish allow it only for a command that declares no connection, relations or statements, and the engine's per-item intent is its replay guard. A command with SQL still names claim, state or inherited. The fixture is [`edge_samples`](../../apps/edge_samples/README.md), which declares the stateless command `sample.read` beside its SQL commands. An application is one component (owner rule, 2026-09-24), and the dev loop refuses a second one, so that component still imports `wamn:postgres` and an edge box cannot load it.

The application that an edge box runs is [`edge_device`](../../apps/edge_device/README.md) (`wamn-10nk`). It declares no model, relation, connection, base dependency or migration, and its one command `sample.read` is stateless. Generate, the data-access derivation, the migration plan, apply-package and the dev loop accept such a package, and only such a package. Its data-access contribution names no schema and no relation, and the dev loop names the host schema after the package. The edge tests load its bundle in place of a hand-encoded guest.
...
`wamn dev edge-bundle --config <environment>/dev.json --out <directory>` writes the bundle from the release that the dev loop published (`wamn-10nk`). It copies the canonical manifest, the admitted facts and the admitted component bytes of the loop's local release, and the ingress guest that the loop serves. `grants.json` gives the platform role `route-caller` the permission of every operation that an attachment serves, as publish grants it in the cloud. The format lives in `wamn_catalog::edge_bundle`, which the writer and `EdgeRelease::load` share.
...
- The platform side is the application [`edge_samples`](../../apps/edge_samples/README.md) (`wamn-at15`). Its command `sample.record` takes the forward's item as it is and is idempotent by claim on `value.idempotency_key`, so a repeated forward answers the first `sample_id` and adds no row. A model create does not fit, because it takes a flat item with no `value` member.
