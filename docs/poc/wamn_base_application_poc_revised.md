...
generator and toolchain provenance
```

Source commit attribution is recorded at Publish and release deployment rather
than embedded in tracked generated bytes. A later additive base schema may
satisfy an unchanged client overlay package’s required contracts without
changing that overlay package’s bytes.
