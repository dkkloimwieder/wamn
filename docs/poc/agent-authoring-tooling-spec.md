...
| F14 | Claim law: idempotent replay returns the immutable original result; a changed request under the same key typed-refuses. Every command declares `idempotent_by` and the generator emits the artifact that value names: `<op>.claim-tests.json` carries the two cases, `<op>.state-tests.json` carries the one case for a command guarded by `expected_row_version`, and `<op>.inherited-tests.json` carries the one case for a command that rides a base command's claim. A command declaring no value refuses generation. [amended: `wamn-10yt.26`] Executing them needs a live database, which is `wamn-f89v`. | `docs/poc/wamn_wms_application_poc_scenario.md:198-209`, `crates/schema/generator/src/generate.rs`, bead `wamn-10yt.19` |
...
6. `--replay <dir>`: runs steps 2 to 4 against a recorded run directory with no
   ...
   `cargo test -p wamn-integration-tests --lib --locked --offline agent_pilot:: -- --nocapture`.
