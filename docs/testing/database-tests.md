...
- The same test makes sure that a second writer waits on the row lock of the first and that the entries follow the serialized changes. A numeric scale-only update moves the stamps and writes an entry.
