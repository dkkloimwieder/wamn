...
- The [operation grant tests](../../crates/control/provision/src/operation_grants.rs) make sure that the Receiving history read is its own operation grant. The [router driver tests](../../crates/execution/host/src/router_driver.rs) make sure that the host refuses a caller without that grant.
