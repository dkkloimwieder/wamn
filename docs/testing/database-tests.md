...
- The [deploy SQL test](../../crates/control/provision/tests/deploy_sql_authority.rs) writes through the stamp function as the guest role, including the `actor-required` refusal. It also reads the stable `wamn_audit_retention` role that `CATALOG_SCHEMA_SQL` creates. It makes sure that `SYSTEM_SCHEMA_SQL` creates no role and raises no error, as `wamn_system` and as a superuser.
- The same deploy SQL test reads the fixed history table shape, the function grants, and the 64-byte name refusal from the server catalogs. It makes sure that `wamn_app` gets its history grants from `record-history-app-grants.sql` alone. `SYSTEM_SCHEMA_SQL` grants nothing to `wamn_app`, and the grant file refuses a database without that role.
- The same test writes logged relations through the guest role. Each change writes one entry with its images, tenant, operation, actor, and transaction id. A key change writes a delete and an insert entry, and a true no-op writes none.
- The same test makes sure that the operation CHECK refuses every other value and rolls back the business write. An unbound operation or actor and a relation with no primary key refuse the write. The guest cannot update or delete an entry.
- The same test makes sure that a second writer waits on the row lock of the first and that the entries follow the serialized changes. Two open writers of different rows commit out of position order, and each entry keeps its own position. A rolled-back competitor leaves no entry. A numeric scale-only update moves the stamps and writes an entry.
- The same test folds the Receiving history read at each retained position, after an insert, two updates, and a delete of a purchase order. It also makes sure that the audit retention role reaches no `app_system` history table.
- The [fold tests](../../apps/platform/data/record-history/tests/fold.rs) reconstruct a complete chain, a truncated chain, a deleted row, a key change, and a row with no retained entries. They keep every value spelling, keep only the retained columns, and refuse rows that do not form one read.
- The [apply-package test](../../crates/control/lib/tests/apply_package_live.rs) installs and removes triggers from declarations, and it makes sure that operation grants stamp `wamn:apply-package`. It also creates history tables, moves log triggers with their retention arguments, keeps a history table when its retention becomes `"none"`, and reads the CDC exclusion rows. It reads the audit retention grants after each retention change, and a replay removes a wider grant.
- The [record history retention test](../../services/ctl/tests/prune_record_history_live.rs) runs `prune-record-history` as the audit retention generation against a package with 30-day, 7-day, and unlimited relations. It makes sure that the verb removes only the expired prefix of each row and writes no marker. It also makes sure that the verb refuses another login and another tenant, and that the role reads no image column. A second case holds the audit retention lock while the verb and an apply-package retention change wait on it. The verb deletes with the retention that it read, and apply-package changes the trigger after that delete commits.
...
- The [introspection test](../../crates/schema/introspection/tests/postgres_live.rs) installs a history table and both record history triggers. It makes sure that the schema description bytes do not change.
- The [package data-access test](../../crates/control/lib/tests/package_data_access_live.rs) writes a logged relation with no stamp columns through the production App role, and it makes sure that the reconciler keeps the history insert grant.
- The [app_system schema test](../../crates/identity/project-state/tests/schema.rs) tests the platform rows, their pinned ids, and the provisioning stamps. It makes sure that each `app_system` relation logs every row change, including a key change, a cascaded delete, and `api_keys.key_hash`. It makes sure that `wamn_history.row_image` spells each `timestamptz` as the platform canonicalizer spells it.
- The [app_system authority test](../../crates/identity/project-state/tests/authority.rs) makes sure that an App generation reads no `app_system` history table. It holds `INSERT` only on the entry columns of `configurations_history`, and it cannot choose a `position`.
- The [generation tests](../../crates/schema/generator/tests/generation.rs) refuse each invalid declaration, retention, and history name, and keep the revision on a true no-op. They also make sure that a logged relation keeps the schema state id. A custom operation reads a history table with only its select grant.
- The [operation grant tests](../../crates/control/provision/src/operation_grants.rs) make sure that the Receiving history read is its own operation grant. The [router driver tests](../../crates/execution/workflow/src/router_driver.rs) make sure that the host refuses a caller without that grant.
- The [claims lint](../../tests/conformance/src/repo_policy/session_claims.rs) in `tools/repo-lint` refuses every reader of `app.user_id` and `app.operation` except the named trigger functions.
...
Development package reconciliation of a logging package remains unexecuted, as [application tests](application-tests.md#record-history) state.
