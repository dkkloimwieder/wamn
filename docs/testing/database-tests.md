...
- The same test folds the history read of the generator fixture at each retained position, after an insert, two updates, and a delete. It also makes sure that the audit retention role reaches no `app_system` history table.
...
- The [generation tests](../../crates/schema/generator/tests/generation.rs) refuse each invalid declaration, retention, and history name, and keep the revision on a true no-op. They also make sure that a logged relation keeps the schema state id. A custom operation reads a history table with only its select grant, and the history read fixture reads every column of its relation.
- The [operation grant tests](../../crates/control/provision/src/operation_grants.rs) make sure that the fixture history read is its own operation grant. The [router driver tests](../../crates/execution/host/src/router_driver.rs) make sure that the host refuses a caller without that grant.
