...
(`docs/events/event-plane-jetstream.md` §10 "curves and ceilings, not verdicts" +
...
always** — the per-level CSVs in `docs/results/ceilings-data/` are the record; the
...
- Aggregated per-level curves: `docs/results/ceilings-data/run{1,2}-ramp-{combined,split-b1,split-b8,split-b32}.csv`,
...
capture (`docs/events/event-plane-jetstream.md`) removes the amplification at the
source, and these numbers stand as the retired path's price tag.

### Raw data

- `docs/results/ceilings-data/c2-trigger.csv`, `c2-bulk.csv`,
...
- `docs/results/ceilings-data/cwal0-perop.csv`, `cwal0-mixed.csv` (extracted from the job
...
- `docs/results/ceilings-data/ce2e-dist.csv`, `ce2e-dist-hist.csv`, `ce2e-fanout.csv`,
...
Raw rows: `docs/results/ceilings-data/ce2e-record-{dist,dist-hist,fanout,burst-depth,burst-applat}.csv`.
...
- `docs/results/ceilings-data/ccdc-drain.csv`, `ccdc-drain-series.csv`, `ccdc-lag.csv`,
