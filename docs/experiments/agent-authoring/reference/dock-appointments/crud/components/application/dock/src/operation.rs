//! The array envelope, and the five operations this component exports.
//!
...
//! own transaction.
//!
//! One item shape serves both. An item IS its body, beside the `request_id`
//! that correlates the answer, and a command carries its idempotency key there
//! too. The other legal choice nests a command body under a `value` member.
...
use crate::{
    appointment_book, appointment_check_in, appointment_query, carrier_create, dock_create,
};
...
/// The command body: the item itself, less its correlation id.
///
/// This package declares every command field at the TOP LEVEL of the item, so
/// a command body reads exactly as a read body does. The other legal choice is
/// to nest the fields under a `value` member, and the published input contract
/// is what says which one an operation chose.
fn command<T: DeserializeOwned>(body: Map<String, Value>) -> Result<T, AccessError> {
...
pub async fn carrier_create_operation(input: &str) -> Result<String, InvocationError> {
    let items = prepare_envelope(input)?;
    let mut output = Vec::with_capacity(items.len());
    for item in items {
        let result = match command::<carrier_create::CreateCarrierCommand>(item.body) {
            Ok(command) => carrier_create::execute(&command).await,
            Err(error) => Err(error),
        };
...
/// Execute only `dock.create`.
///
/// # Errors
///
/// [`InvocationError`] when the ENVELOPE is inadmissible.
pub async fn dock_create_operation(input: &str) -> Result<String, InvocationError> {
    let items = prepare_envelope(input)?;
    let mut output = Vec::with_capacity(items.len());
    for item in items {
        let result = match command::<dock_create::CreateDockCommand>(item.body) {
            Ok(command) => dock_create::execute(&command).await,
            Err(error) => Err(error),
        };
        output.push(outcome(item.request_id, result));
    }
...
/// Execute only `appointment.book`.
///
/// # Errors
///
/// [`InvocationError`] when the ENVELOPE is inadmissible.
pub async fn appointment_book_operation(input: &str) -> Result<String, InvocationError> {
    let items = prepare_envelope(input)?;
    let mut output = Vec::with_capacity(items.len());
    for item in items {
        let result = match command::<appointment_book::BookCommand>(item.body) {
            Ok(command) => appointment_book::execute(&command).await,
            Err(error) => Err(error),
        };
        output.push(outcome(item.request_id, result));
    }
...
/// Execute only `appointment.check_in`.
///
/// # Errors
///
/// [`InvocationError`] when the ENVELOPE is inadmissible.
pub async fn appointment_check_in_operation(input: &str) -> Result<String, InvocationError> {
    let items = prepare_envelope(input)?;
    let mut output = Vec::with_capacity(items.len());
    for item in items {
        let result = match command::<appointment_check_in::CheckInCommand>(item.body) {
            Ok(command) => appointment_check_in::execute(&command).await,
            Err(error) => Err(error),
        };
        output.push(outcome(item.request_id, result));
    }
...
/// Execute only `appointment.query`.
///
/// # Errors
///
/// [`InvocationError`] when the ENVELOPE is inadmissible.
pub async fn appointment_query_operation(input: &str) -> Result<String, InvocationError> {
    let items = prepare_envelope(input)?;
    let mut connection = Connection::new();
    let mut output = Vec::with_capacity(items.len());
    for item in items {
        let result = match read::<appointment_query::QueryInput>(item.body) {
            Ok(input) => appointment_query::execute(&mut connection, &input).await,
            Err(error) => Err(error),
        };
        output.push(outcome(item.request_id, result));
    }
...
mod tests {
    ...
    fn the_correlation_id_is_the_envelopes_and_the_body_is_the_items() {
        ...
        let error = read::<appointment_query::QueryInput>(items[0].body.clone()).unwrap_err();
        assert_eq!(error.kind(), AccessErrorKind::InvalidInput);
        let error = command::<appointment_book::BookCommand>(items[1].body.clone()).unwrap_err();
        assert_eq!(error.kind(), AccessErrorKind::InvalidInput);
        assert!(
            error.detail()["field"]
                .as_str()
    ...
    fn a_refusal_serializes_as_code_and_detail_beside_its_request_id() {
        ...
        assert_eq!(
            serialized(&output),
            r#"[{"request_id":"r","error":{"code":"not_found","detail":{"field":"appointment_id","id":"x"}}}]"#
        );
    }
}
