...
| R8c | Outbox write-amplification + GC verification | Medium — scale | **Closed 2026-07-18** — D19 v3 (CDC) retires the outbox capture path; GC had shipped (wamn-d8v), amplification is moot (wamn-vbl/wamn-32d closed; measured record in `docs/archive/results/ceilings.md` § C2) |
...
5. Doc comments in both files + `docs/archive/execution/run-queue.md` lifecycle step 3: state the crash-evidence semantics explicitly, including the first-dispatch case (first claim is not a redelivery; a first-dispatch crash costs its unit on the *reclaim*).
...
### Verification
- Re-run the full S2 gate set (throughput, p99, chaos ×100, RLS ×10k, injection ×10k) — thresholds unchanged; record the new p50/p99 next to the old in `docs/archive/results/p0-results.md` as an addendum so the round-trip cost is on the record.
...
### Doc closure
`docs/archive/data-path/security-db-path.md` + the WIT header comment: the "no interpolation path" claim becomes unconditionally true; note the identity-format validators' demoted role.
...
Record in the fork branch's final commit message (or `docs/archive/results/p0-results.md` addendum): date, base rev old→new, commits carried/dropped, gate numbers old→new. Budget: an afternoon when clean.
...
**Amendment (2026-07-18, D19 v3).** The trigger path this finding describes is retired, but the silent-loss *class* recurs at CDC decode: registrations keyed on physical table names break the same way when a rename changes the decoded relation name. The residual closes at **wamn-l5i9.11 [EVT-OIDMAP]** — OID→entity-id keying at decode makes registrations rename-stable (with a rename drill as the gate). The 11.8 impact-analysis case remains valid regardless of capture mechanism.

**Closure (2026-07-19, wamn-l5i9.11).** The **decode side** is closed. The reader resolves each relation OID to its stable catalog entity id through a `wamn_entities` map (`relation_oid → entity_id, table_name`) that publish/migrate-catalog maintain IN the DDL transaction; the map is OID-keyed, and pg_class OIDs survive `ALTER TABLE RENAME`, so a rename only updates the informational `table_name` while the entity id (and OID) hold. Envelopes carry the entity id in `entity` and the physical name in `table`; the subject's entity segment is the id — a registration's JetStream consumer filter is therefore rename-proof. Verified by a **live rename drill** (entity provisioned + renamed through the real `migrate-catalog` path in one reader session, OID asserted constant, stable id on every envelope/subject across the rename) and **5 mutants** (map upsert dropped from migrate / from publish, reader lookup bypassed, subject keyed by table, upsert loses `ON CONFLICT`). **Still open (materializer, l5i9.17):** the *registration keeps firing* half — the drill asserts the envelope/stream side only; that a durable consumer keyed on the entity id continues delivering across a rename lands with the materializer's own gate. The 11.8 impact-analysis case stands.
...
7. **R8b** (dispatch role) + the NATS `verify_and_map` scoping — next dispatcher/deploy touch, one commit (the replication-credential tier is separate and rides the Phase-1 CDC reader, wamn-l5i9.10). **R8c** — closed 2026-07-18 (D19 v3; see the section amendment). **R8a** — decide now, implement before 9.6 retention exists. **R8d** — doc line now, knob later.
8. **R9a** — one validation rule, next 3.1 touch. **R9b** — decode side **closed 2026-07-19 (wamn-l5i9.11, EVT-OIDMAP)**: OID→entity-id keying makes envelopes + subjects rename-stable; the registration-continuity half rides the materializer (l5i9.17) and the 11.8 spec case stands (its remediation policy — auto-migrate vs re-register — is a small decision to take when filing). **R9c** — one doc paragraph now; the residue janitor is deferred to the non-txn migration work it belongs to. `wamn-drb` / `wamn-nqg` remain tracked in-repo.
