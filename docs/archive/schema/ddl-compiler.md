...
> capture is CDC via logical decoding (`docs/archive/events/event-plane-jetstream.md`): the
...
> (b687d45) and the C2 trigger-overhead campaign in docs/archive/results/ceilings.md. What
...
- Plan: `docs/archive/platform-plan.md` §Epic 3 (3.2), 2.5 (migration engine), D14.
- Catalog model (the input): `docs/archive/schema/catalog-model.md`, `crates/schema/model`.
- Tenant shape: `deploy/sql/postgres-init.sql`, `docs/archive/data-path/security-db-path.md`.
