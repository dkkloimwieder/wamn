...
- **Pure crate:** `crates/schema/control/src/impact` — the decision (`analyze` → `ImpactReport`).
...
`wamn_schema_control::impact::analyze(&ImpactInput) -> ImpactReport` groups the migration by
...
1. **affected entity + classification** — group the compiled plan's operations by
   `wamn_schema_compiler::Operation::entity`; an entity is destructive iff any of its ops is
...
- Pure unit tests (`crates/schema/control/src/impact`) carry the bulk: the touched/untouched
...
- Live gate: `services/ctl/tests/impact_report_live.rs` (throwaway PG). In-cluster
  ...
  `docs/archive/build-and-test.md` [11.8].

## References

- Plan: `docs/archive/platform-plan.md` §Epic 11 (11.8).
- The plan model (the input): `docs/archive/schema/ddl-compiler.md`, `crates/schema/compiler`.
- The registration edge: `crates/events/registration`, `docs/archive/build-and-test.md` [EVT-REG/D24].
- The suite storage: `docs/archive/testing/scenario-catalog.md`, `deploy/sql/authoring-tests.sql`.
