...
It is the foundation of the Postgres topology (`docs/archive/platform/postgres-topology.md`,
...
generic deployment model** (`docs/archive/platform/deployment-model.md`): `env` is a validated
...
`docs/archive/platform/deployment-model.md`.
...
See the `[D6/wamn-q3n.1]` / `[D6/wamn-q3n.3]` blocks in `docs/archive/build-and-test.md`
for the exact commands (`cargo test -p wamn-control-registry` + clippy/fmt + the
live-apply gates).

## References

- Topology: `docs/archive/platform/postgres-topology.md` (§T1, §Environments, §Reversibility).
