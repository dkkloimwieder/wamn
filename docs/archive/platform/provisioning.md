...
> See `docs/archive/platform/postgres-topology.md` for the decision and the trade-off analysis.
...
> `cluster_of`. See `docs/archive/platform/deployment-model.md`.
...
The topology (`docs/archive/platform/postgres-topology.md`) splits `provision-project` into
...
`wamn-pg` — the T3-style tier, `docs/archive/platform/postgres-topology.md` §T3), so it owns no
...
path (`docs/archive/platform/deployment-model.md`):
...
(docs/archive/platform/postgres-topology.md §Backup architecture); the per-project-env logical dump
...
(the scratch-cluster runbook, docs/archive/platform/postgres-topology.md). The formal restore **drill**
...
(docs/archive/events/event-plane-jetstream.md §4). CDC is opt-in and may be enabled long after
...
registrations (docs/archive/events/event-plane-jetstream.md §5, "Old images"). `FULL` is a
...
The **second backup mechanism** (docs/archive/platform/postgres-topology.md §Backup architecture):
...
(`docs/archive/platform/deployment-model.md` §4). The step plan is the pure
...
in-cluster cross-cluster move standup; see `docs/archive/build-and-test.md`
`[ARCH/wamn-8df.5]`.
...
`copy(src → dst)` operation** (`docs/archive/platform/deployment-model.md` §4), shipped as
...
See the `[2.3]` and `[D6/wamn-q3n.*]` blocks in `docs/archive/build-and-test.md` for the
exact commands (pure crate unit + live-apply, the in-cluster `provisionbench`
gate of record, and the CNPG standups).

## References

- Plan: `docs/archive/platform-plan.md` §2.3, D6.
- Credential seam: `crates/platform/runtime/src/plugins/wamn_postgres/mod.rs`
  (`CredentialProvider` / `StaticCredentialProvider` / `ProjectConfig`).
- D6 decision: `docs/archive/platform-plan.md` decision table.
