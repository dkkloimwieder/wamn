...
- **Shipped baseline being amended:** one shared `Cluster` `wamn-pg`,
  database-per-project, imperative `provision-project` (`docs/archive/platform/provisioning.md`).
...
alongside the T3 pool. `docs/archive/platform/system-cluster.md`. The registry tables + the four
...
separate concern (`wamn-689`). `docs/archive/platform/provisioning.md`. *(Encoding superseded by
...
lives on its own cluster, physically isolated from prod). `docs/archive/platform/provisioning.md`
...
recovery domain, derived by `cluster_of`; see `docs/archive/platform/deployment-model.md`.)*
...
> `cluster_of`. See `docs/archive/platform/deployment-model.md`. The topology (T1–T4) is
...
1. **`provision-org`** (new): render the T2 `Cluster` pair CRs (prod: HA per
   ...
   *Shipped (`wamn-q3n.6`):* the org **cluster-pair renderer**
   ...
   store prefix) is deferred to `wamn-e1g`. `docs/archive/platform/provisioning.md`.
2. **`provision-project-env`**: create the project-env database + roles
   ...
   *Shipped (`wamn-q3n.7`):* the `provision-project-env` subcommand + the pure
   ...
   `docs/archive/platform/provisioning.md`.
3. **`provisionbench`** extends to the org pair + a T3 path; the saga records
   ...
   *Shipped (`wamn-q3n.8`):* `provisionbench --mode` (`legacy`/`orgpair`/`t3`/
   ...
   org-pair standup gate of record. `docs/archive/platform/provisioning.md`.
...
(`pg_dump -Fd` → `pg_restore` into a scratch DB). See docs/archive/platform/provisioning.md
...
replace) and by an in-cluster restore standup on `wamn-pg`. See docs/archive/platform/provisioning.md
...
re-applying the already-running clusters here. See docs/archive/platform/provisioning.md §`provision-org`.
...
by `wamn-q3n.14`). See `docs/archive/platform/provisioning.md` §`move-org-tier`. *(Retired by
...
(`…/backup_barmanobjectstore/` + plugin repo). wamn: `docs/archive/platform/provisioning.md`,
