...
(`docs/archive/platform-plan.md` §9.1); logs (S5) already ship the same way to Loki.
...
- **Host-enforced outbound inject.** wash-runtime already *extracts* an incoming
  ...
  see `docs/archive/platform/wash-runtime-fork.md`): it stamps the current W3C trace context onto
...
- Plan: `docs/archive/platform-plan.md` §9.1; D13 (`Observability store`).
- S5 logging (the sibling signal + collector/Loki manifests):
  `docs/archive/results/p0-results.md` S5.
- The fork observability init: `docs/archive/platform/wash-runtime-fork.md`.
