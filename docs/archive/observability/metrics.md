...
(9.1, `docs/archive/observability/tracing.md`) and logs (S5). This is the metrics third of Epic 9
(`docs/archive/platform-plan.md` §9.8).
...
| `wamn.postgres.pool.{size,available,waiting}` | observable gauge (u64) | `wamn.project`, `wamn.pool.lifecycle` (`guest`/`platform`) | `WamnPostgres::register_pool_metrics` (deadpool `Pool::status()`) |
...
Run recipe (local + in-cluster gate of record): `docs/archive/build-and-test.md` §[9.8].
