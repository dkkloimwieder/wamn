...
dashboards third of Epic 9 (`docs/archive/platform-plan.md` §9.9). It adds **no new
...
`docs/archive/observability/metrics.md` — a renamed or corrupted metric fails the build, not the
...
- The registry (`crates/control/registry`, `deploy/sql/system-schema.sql`) models
  ...
  (`docs/archive/platform/postgres-topology.md`).
...
Local iteration + the in-cluster gate of record: `docs/archive/build-and-test.md` §[9.9].
...
- Plan: `docs/archive/platform-plan.md` §9.9.
- The three sinks: `docs/archive/observability/tracing.md` (9.1), `docs/archive/observability/metrics.md` (9.8), S5 logging.
- Registry model: `docs/archive/platform/registry-model.md`, `deploy/sql/system-schema.sql`.
