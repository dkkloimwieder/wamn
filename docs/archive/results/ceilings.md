...
(`docs/archive/events/event-plane-jetstream.md` §10 "curves and ceilings, not verdicts" +
...
## PLAN-3-F1 — capture-on run-state baseline (wamn-vshi.5)

**Date:** 2026-07-31 America/New_York (2026-08-01 UTC) · **Source base:**
`57016e5` plus the `wamn-vshi.5` implementation tree · **Exact gates image:**
local build `sha256:58cb84f6ba9dbf9fef1591b9eb5e428532c4e04c74377c5d6517090d61b8ebda`,
kind import image ID
`sha256:30e3ea16275ab9fc201b81054f99e3063a8b42eae9cc9cb56d78f71b464d7279`.

**Question:** what does the current capture-replay persistence model cost before
PLAN item 1 replaces it with boundary checkpoints? This is the denominator for
the item-1 before/after comparison, not an R6 verdict or a payload-threshold
decision.

**What one run is:** the canonical successful F1 `receipt-received` path — eight
node boundaries (`request` through `respond`) and the real F1 payload transforms,
at 1, 10, and 100 schema-valid receipt lines. Every boundary commits full input
and output JSON to `node_runs`; the five context-bearing nodes update
`runs.state_json`; admission and completion write the run input and result. The
campaign applies the production `deploy/sql/run-state.sql` DDL under a renamed
ephemeral schema, including indexes, RLS, and triggers, then drops the schema.
It deliberately does not execute node computation or F1 business-table writes:
the reported boundary latency and storage are the run-state persistence
component, isolated from connector and business-SQL cost.

**Environment:** 3-node kind reference cluster on the shared developer host;
gates Job pod-affined to the fixture PostgreSQL 18 pod; `fsync=on`,
`synchronous_commit=on`; no CPU limit; stock table/autovacuum settings; four
concurrent writers; 5,000 completed runs and 40,000 full-capture node rows per
point. The harness opens an application connection per run, outside every
timed boundary; consequently `runs/s` describes campaign pacing and must not be
read as runner or pool capacity. Each point starts from `TRUNCATE`, reset table counters, `VACUUM
(ANALYZE)`, and `CHECKPOINT`. WAL is bracketed with
`pg_current_wal_insert_lsn`, so it measures generated bytes. Relation growth is
the change in `pg_total_relation_size`; heap and TOAST columns are terminal
absolute sizes after the normalized empty starting point.

| F1 lines | Campaign runs/s | Boundary p50 / p99 | WAL/run | `runs` growth | `node_runs` growth | Terminal TOAST | Dead tuples | Autovacuums |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 1 | 88.5 | 3.73 / 6.36 ms | 12.67 KiB | 4.57 MiB | 33.91 MiB | 0 | 2,646 | 2 |
| 10 | 87.8 | 3.73 / 6.14 ms | 22.92 KiB | 26.35 MiB | 46.92 MiB | 0 | 3,523 | 2 |
| 100 | 64.2 | 5.93 / 10.01 ms | 35.87 KiB | 34.98 MiB | 88.01 MiB | 65.36 MiB | 4,814 | 2 |

**Reading:** the 1→10-line increase is absorbed without a latency or throughput
knee in timed boundary latency, but WAL grows by 81%. At 100 lines the current
model crosses the TOAST boundary: combined run-state growth reaches about 123
MiB for 5,000 runs, roughly 65 MiB of it TOAST; campaign completion rate falls
27% from the one-line point and boundary p99 rises 57%. The run row receives six updates per execution (five
context replacements plus terminal completion), yielding 30,000 updates and a
visible dead-tuple/autovacuum burden at every point. These are the mechanisms
item 1 predicts checkpoint-only recovery and capture separation will change.
No budget attaches to these figures.

All provenance sanity checks passed: durable commit settings, exactly 5,000
runs, exactly 40,000 successful node rows, every node row full-captured with
both JSON values, nonzero WAL, Job success, and zero residual
`wamn_plan3_baseline` schemas. Raw data:
`docs/archive/results/ceilings-data/plan3-f1-capture-baseline.csv`. Reproduce via
`docs/archive/build-and-test.md` § `[PLAN-3 / wamn-vshi.5]`.
...
- Reproduce: `docs/archive/build-and-test.md` § [EVT-C7 / wamn-z7b.1].
...
capture (`docs/archive/events/event-plane-jetstream.md`) removes the amplification at the
source, and these numbers stand as the retired path's price tag.

### Raw data

- `docs/archive/results/ceilings-data/c2-trigger.csv`, `c2-bulk.csv`,
...
- Reproduce: `docs/archive/build-and-test.md` § [EVT-C2 / wamn-z7b.2].
...
- `docs/archive/results/ceilings-data/cwal0-perop.csv`, `cwal0-mixed.csv` (extracted from the job
  log's `=== BEGIN CSV … ===` blocks).
- Reproduce: `docs/archive/build-and-test.md` § [EVT-C-WAL-0 / wamn-l5i9.4].
...
- Reproduce: `docs/archive/build-and-test.md` § [EVT-MAT / wamn-l5i9.17].
...
- `docs/archive/results/ceilings-data/ce2e-dist.csv`, `ce2e-dist-hist.csv`, `ce2e-fanout.csv`,
  `ce2e-burst-depth.csv`, `ce2e-burst-applat.csv` (run 1 — the clean run;
  extracted from the job log's `=== BEGIN CSV … ===` blocks).
- Reproduce: `docs/archive/build-and-test.md` § [EVT-C-E2E / wamn-l5i9.22].
...
- `docs/archive/results/ceilings-data/ccdc-drain.csv`, `ccdc-drain-series.csv`, `ccdc-lag.csv`,
  `ccdc-lag-series.csv`, `ccdc-ri.csv` (extracted from the record job log's
  `=== BEGIN CSV … ===` blocks).
- Reproduce: `docs/archive/build-and-test.md` § [EVT-C-CDC / wamn-l5i9.14].
