...
| ~~Which project-database authority admits management-produced runs?~~ | **Settled (`wamn-0h0g.7.5`, `docs/plane-amendment.md`):** stable host-only ACL role `wamn_management_admitter` with scoped A/B LOGIN generations uses the existing credential lifecycle and extends the sole native `wamn-run-state` admission API. It atomically creates ordinary run + queue facts with stable producer identity and draft-run `full` or test-case `off` capture; exact retries return existing and divergent facts refuse. It has no post-admission mutation or unrelated authority, and `wamn_app`, guests, author SQL, `wamn_scenario_author`, and `wamn_effect_writer` remain denied. | — |
