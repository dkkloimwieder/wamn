...
WIT variant (`docs/archive/contracts/wamn-node.wit`); the engine's action for each is fixed:
...
| The standard node *contents* (incl. the Postgres/raw-SQL node, D8) | 5.3 — SHIPPED: `wamn-standard-nodes` (docs/archive/execution/node-library.md) |
