...
| Crate | What it is |
|---|---|
| `crates/node/sdk` | The node **authoring contract** — the `Node` trait, the `RunContext` view of a dispatch, the `NodeCtx` capability facade every effect flows through, and the `wamn:node` error taxonomy (`NodeError`/`ErrorDetail`/`RateLimitDetail`, now DEFINED here and re-exported by `wamn-runner`). A Rust mirror of the drafted `docs/archive/contracts/wamn-node.wit`; the 5.4 freeze layers the WIT + guest scaffolding on top. |
