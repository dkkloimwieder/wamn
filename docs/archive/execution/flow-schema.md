...
- **Contract file:** [`flow-schema.schema.json`](../contracts/flow-schema.schema.json) — the
...
executions, `docs/archive/execution/flow-runner.md` § *Dispatch budget*) fails a loop that never
...
During the pre-release MVP reduction, `0.1` is the exact allowlist: retired
ordering and partition fields are rejected rather than grandfathered. After
that contract freezes, additive clarifications may remain `0.1.x`; a breaking
change waits for `0.2`. The validator rejects a `schema-version` with a newer
major or minor than it implements.
...
- Plan: `docs/archive/platform-plan.md` §Epic 5 (5.1–5.3, 5.14), D1 (flow-as-IR).
- POC flows: `docs/archive/poc/poc-material-receiving.md` (F1–F4).
- Node contract: `docs/archive/contracts/wamn-node.wit` (payload/config/credential handles).
- S3 stand-in: `components/execution/flowrunner`, `docs/archive/results/p0-results.md` S3.
