...
> See `docs/platform/postgres-topology.md` for the decision and the trade-off analysis.
...
> `cluster_of`. See `docs/platform/deployment-model.md`.
...
The topology (`docs/platform/postgres-topology.md`) splits `provision-project` into
...
`wamn-pg` — the T3-style tier, `docs/platform/postgres-topology.md` §T3), so it owns no
...
path (`docs/platform/deployment-model.md`):
...
(docs/platform/postgres-topology.md §Backup architecture); the per-project-env logical dump
...
(the scratch-cluster runbook, docs/platform/postgres-topology.md). The formal restore **drill**
...
(docs/events/event-plane-jetstream.md §4). CDC is opt-in and may be enabled long after
...
registrations (docs/events/event-plane-jetstream.md §5, "Old images"). `FULL` is a
...
The **second backup mechanism** (docs/platform/postgres-topology.md §Backup architecture):
...
(`docs/platform/deployment-model.md` §4). The step plan is the pure
...
`copy(src → dst)` operation** (`docs/platform/deployment-model.md` §4), shipped as
