...
pattern, as `docs/platform/postgres-topology.md` preceded provisioning); the implementation
...
dimension (`docs/platform/postgres-topology.md` §Environments); the four-tier topology
...
`docs/platform/postgres-topology.md`'s T1–T4 topology is **unchanged as deployment reality**;
...
`docs/platform/postgres-topology.md` §Environments and the `platform-plan.md` D6 row are
...
`docs/platform/postgres-topology.md` (the four tiers, the `CredentialProvider` seam this
copy/repoint rides), `docs/platform/registry-model.md` (the identity model this widens),
`docs/platform/provisioning.md` (`provision-org`/`-project-env`/`dump`/`restore`/`move-org-tier`
generalized here), `docs/schema/schema-lifecycle.md` (3.4 promote = the definition half of
