...
// wamn-hopk R3: the artifact GRANT-statement scan lived here - a comment
// stripper, a `;`-splitting statement parser, and the two tests reading
// their output. It is deleted, not repointed. The control author's
// allowlist is enforced live by AUTHORING_ROLE_PROBE_SQL
// (services/scenario-worker/src/authoring.rs:273), which refuses AT STARTUP
// if that role holds write privilege outside the two-row allowlist. The
// text scan was a weaker pre-echo of it. The shared-posture check that also
// stood here read five other test files as source; its behavioural analogue
// is `current_database_connect_posture_kills_remove_and_sibling_scope_mutants_on_postgres`
// below, and the two-tenant authority proof is
// `control_author_two_tenant_authority_holds_on_postgres`.
