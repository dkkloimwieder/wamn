...
fn column_clause(name: &str, field: &wamn_schema_model::Field) -> String {
...
fn fk_op(entity: &Entity, field: &wamn_schema_model::Field, target_table: &str) -> Operation {
    let t = &entity.name;
...
/// the DB).
fn unit_comment_op(entity: &Entity, field: &wamn_schema_model::Field, unit: &str) -> Operation {
    let t = &entity.name;
    let sql = format!(
...
pub(crate) fn migrate_plan(old: &Catalog, new: &Catalog) -> Result<MigrationPlan, CompileError> {
    ...
    let d = wamn_schema_model::diff(old, new);
    let mut plan = MigrationPlan::default();
    ...
    let mut hoisted_col_fields: HashMap<&str, Vec<&wamn_schema_model::Field>> = HashMap::new();
    for ch in &d.entities_changed {
        let old_e = old_by_id[ch.id.as_str()];
        let new_e = new_by_id[ch.id.as_str()];
...
fn field_by_id<'a>(entity: &'a Entity, id: &str) -> Option<&'a wamn_schema_model::Field> {
...
fn emit_additive_changes(
    ...
    old_e: &Entity,
    new_e: &Entity,
    ch: &wamn_schema_model::EntityChange,
...
fn drop_column_op(new_e: &Entity, f: &wamn_schema_model::Field) -> Operation {
    Operation {
...
mod tests {
    ...
    use crate::CompileError;
    use wamn_schema_model::{Catalog, Entity, Field, FieldType, Index};
