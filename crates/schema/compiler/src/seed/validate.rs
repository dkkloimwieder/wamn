...
use wamn_schema_model::{Catalog, Constraint, Entity, FieldType, Issue, Severity};

use super::model::{Dataset, SCHEMA_VERSION};
...
pub fn validate(dataset: &Dataset, catalog: &Catalog) -> Result<(), Vec<Issue>> {
    ...
    for (ei, es) in dataset.entities.iter().enumerate() {
        ...
        let by_name: HashMap<&str, &wamn_schema_model::Field> =
