...
fn migration_with_name_reuse_applies_on_postgres() {
    ...
    script.push_str(
        "DO $$ BEGIN IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname='wamn_app') THEN \
         BEGIN CREATE ROLE wamn_app LOGIN PASSWORD 'wamn_app' NOSUPERUSER NOCREATEDB NOBYPASSRLS; \
         EXCEPTION WHEN duplicate_object OR unique_violation THEN NULL; END; END IF; END $$;\n\
         DROP SCHEMA IF EXISTS wamn_schema_compiler_reuse_test CASCADE;\n\
         CREATE SCHEMA wamn_schema_compiler_reuse_test AUTHORIZATION CURRENT_USER;\n\
    ...
    script.push_str(
        "DO $$ BEGIN\n\
             ...
             ASSERT (SELECT count(*) FROM information_schema.columns WHERE table_schema = 'wamn_schema_compiler_reuse_test' AND table_name = 'receipts_old' AND column_name = 'note') = 1, 'rename + add-column on one entity applied';\n\
             ...
             ASSERT (SELECT count(*) FROM pg_indexes WHERE schemaname = 'wamn_schema_compiler_reuse_test' AND tablename = 'audit' AND indexname = 'audit_pkey') = 1, 'recreated audit owns the canonical pkey name';\n\
             ASSERT (SELECT count(*) FROM pg_indexes WHERE schemaname = 'wamn_schema_compiler_reuse_test' AND tablename = 'audit' AND indexname = 'audit_by_val') = 1, 'recreated audit owns the reclaimed index name';\n\
             ASSERT (SELECT count(*) FROM pg_indexes WHERE schemaname = 'wamn_schema_compiler_reuse_test' AND tablename = 'receipts_old' AND indexname = 'receipts_old_pkey') = 1, 'renamed table pkey followed the rename';\n\
             ASSERT (SELECT count(*) FROM pg_indexes WHERE schemaname = 'wamn_schema_compiler_reuse_test' AND tablename = 'receipts' AND indexname = 'receipts_pkey') = 1, 'recreated receipts owns the canonical pkey name (no silent suffix drift)';\n\
             ASSERT to_regclass('wamn_schema_compiler_reuse_test.wamn_mig_drop_audit') IS NULL, 'the renamed-aside table was dropped';\n\
             ASSERT (SELECT count(*) FROM information_schema.columns WHERE table_schema = 'wamn_schema_compiler_reuse_test' AND table_name = 'log' AND column_name = 'audit_ref') = 0, 'inbound FK column unwound';\n\
         END $$;\n",
    );
    ...
    script.push_str(
        "DO $$ BEGIN\n\
             ...
             ASSERT (SELECT count(*) FROM pg_constraint WHERE conname = 'audit_val_uniq' AND conrelid = 'wamn_schema_compiler_reuse_test.audit_log'::regclass) = 1, 'constraint redefined on the renamed table';\n\
         END $$;\n",
    );
    ...
    script.push_str(
        "DO $$ BEGIN\n\
         ...
         END $$;\n\
         DROP SCHEMA wamn_schema_compiler_reuse_test CASCADE;\n",
    );
...
fn removed_entity_drops_apply_on_postgres() {
    ...
    script.push_str(
        "DO $$ BEGIN IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname='wamn_app') THEN \
         BEGIN CREATE ROLE wamn_app LOGIN PASSWORD 'wamn_app' NOSUPERUSER NOCREATEDB NOBYPASSRLS; \
         EXCEPTION WHEN duplicate_object OR unique_violation THEN NULL; END; END IF; END $$;\n\
         DROP SCHEMA IF EXISTS wamn_schema_compiler_drop_order_test CASCADE;\n\
         CREATE SCHEMA wamn_schema_compiler_drop_order_test AUTHORIZATION CURRENT_USER;\n\
    ...
    script.push_str(
        "DO $$ BEGIN\n\
             ...
             ASSERT to_regclass('wamn_schema_compiler_drop_order_test.books') IS NULL, 'books dropped';\n\
             ASSERT to_regclass('wamn_schema_compiler_drop_order_test.reviews') IS NULL, 'reviews dropped';\n\
         END $$;\n\
         DROP SCHEMA wamn_schema_compiler_drop_order_test CASCADE;\n",
    );
...
fn reference_retype_fk_lifecycle_applies_on_postgres() {
    ...
    script.push_str(
        "DO $$ BEGIN IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname='wamn_app') THEN \
         BEGIN CREATE ROLE wamn_app LOGIN PASSWORD 'wamn_app' NOSUPERUSER NOCREATEDB NOBYPASSRLS; \
         EXCEPTION WHEN duplicate_object OR unique_violation THEN NULL; END; END IF; END $$;\n\
         DROP SCHEMA IF EXISTS wamn_schema_compiler_retype_fk_test CASCADE;\n\
         CREATE SCHEMA wamn_schema_compiler_retype_fk_test AUTHORIZATION CURRENT_USER;\n\
         GRANT USAGE ON SCHEMA wamn_schema_compiler_retype_fk_test TO wamn_app;\n\
    ...
    script.push_str(
        "DO $$ BEGIN\n\
             ...
             END;\n\
         END $$;\n\
         DROP SCHEMA wamn_schema_compiler_retype_fk_test CASCADE;\n",
    );
...
/// Drift guard (wamn-cjv.9 / review C1-2): wamn-schema-model derives `<table>_pkey`
...
/// catalog must be a member of `wamn_schema_model::synthesized_identifiers`.
#[test]
fn synthesized_identifiers_cover_every_emitted_relation_and_constraint() {
    let c = poc();
    let ids = wamn_schema_model::synthesized_identifiers(&c);
    ...
    let ids2 = wamn_schema_model::synthesized_identifiers(&v2);
    let pkey_op = mig
        .operations
        .iter()
