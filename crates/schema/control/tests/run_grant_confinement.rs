...
#[test]
fn the_execution_pin_cutover_restores_only_ratified_authority() {
    // A legacy table without the claim-record pair takes the pin cutover, which
    // is the migration path every pre-existing project database walks. It used
    // to hand back INSERT+UPDATE on all three columns it touches; the INSERT
    // half of the claim record is ungated (the pin trigger is BEFORE UPDATE),
    // so a blanket restore re-opened on the legacy path exactly what
    // `deploy/sql/run-state.sql` withholds on the fresh one.
    let mut columns = CANONICAL_RUN_COLUMNS.to_vec();
    columns.retain(|column| *column != "release_version" && *column != "manifest_digest");
    let obs = broadly_granted_runs(&columns);
    let plan = plan_run_plane(&schema(), &obs);
    ...
    let cutover = plan
        .actions
        .iter()
        .find(|action| action.kind == RunPlaneActionKind::ExecutionPinCutover)
    ...
    assert!(
        ...
        "cutover restored an unratified grant: {}",
        cutover.sql
    );
    // The two halves that made this a re-widener rather than a restore.
    assert!(
        !cutover
            .sql
            .contains("INSERT (execution_bundle_hash, release_version, manifest_digest)"),
        "the claim record must not become app-INSERTable: {}",
        cutover.sql
    );
    assert!(
        !cutover.sql.contains("UPDATE (execution_bundle_hash"),
        "the admission pin must not become app-UPDATEable: {}",
        cutover.sql
    );
}
