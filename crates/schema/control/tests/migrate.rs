...
fn rollback_of_a_forward_migration_is_the_inverse() {
    ...
    assert!(matches!(
        ...
        Err(wamn_schema_control::RequiresConfirmation { .. })
    ));
...
fn catalog_schema_sql_mirrors_the_engine() {
    ...
    assert!(sql.contains(&format!(
        "'{}'",
        wamn_schema_control::sql::confirmation_sql(Confirmation::None)
    )));
    assert!(sql.contains(&format!(
        "'{}'",
        wamn_schema_control::sql::confirmation_sql(Confirmation::ConfirmedWithBackup)
    )));
