...
fn fixtures_conform_to_published_schema() {
    ...
    // ties docs/reference/contracts/catalog-model.schema.json to the real models the API/designer
    // will send.
...
fn committed_schema_matches_types() {
    ...
    let committed = std::fs::read_to_string(
        Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../../docs/reference/contracts/catalog-model.schema.json"),
    )
    .expect("read committed schema");
    assert_eq!(
        committed,
        wamn_schema_model::json_schema_string(),
        "docs/reference/contracts/catalog-model.schema.json is stale — regenerate it (see print-catalog-model-schema example)"
    );
}
