//! Catalog-derived entity operation planner.
//!
...
//! [`quote_ident`](wamn_pg_core::quote_ident).

use std::collections::HashMap;

use serde_json::Value;
use wamn_pg_core::{SqlValue, Statement, quote_ident};
use wamn_schema_model::{Cardinality, Catalog, Entity, Field, FieldType};

use crate::error::EntityAccessError;
...
pub struct Expansion {
    ...
    pub(crate) dir: ExpansionDirection,
...
impl Expansion {
    ...
    pub fn dir(&self) -> ExpansionDirection {
        self.dir
    }
...
/// What kind of operation a [`EntityPlan`] carries.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[non_exhaustive]
pub enum PlanKind {
    List,
...
/// afterward, and the result cardinality.
#[derive(Debug, Clone, PartialEq)]
pub struct EntityPlan {
    pub(crate) kind: PlanKind,
    pub(crate) statement: Statement,
    pub(crate) expands: Vec<Expansion>,
}
...
impl<'a> Planner<'a> {
    ...
    /// Compile a request into a [`EntityPlan`], or reject it with an [`EntityAccessError`].
    ///
    /// No SQL is produced until every identifier has resolved against the
    /// catalog and every value has typed against its field.
    pub fn plan(&self, request: &EntityRequest) -> Result<EntityPlan, EntityAccessError> {
        let entity = self.entity_by_name(&request.entity)?;
        match &request.operation {
            EntityOperation::List(options) => self.compile_list(entity, options),
            EntityOperation::Get { id, expand } => self.compile_get(entity, id, expand),
            EntityOperation::Create { fields } => self.compile_create(entity, fields),
            EntityOperation::Update { id, fields, mode } => {
                self.compile_update(entity, id, fields, *mode == UpdateMode::Replace)
            }
            EntityOperation::Delete { id } => self.compile_delete(entity, id),
        }
    }
    ...
    pub fn build_expansion(&self, ex: &Expansion, keys: &[SqlValue]) -> Statement {
        ...
        let placeholders = (1..=keys.len())
            ...
            .join(", ");
        Statement::new(
            format!("SELECT {proj} FROM {tbl} WHERE {mcol} IN ({placeholders})"),
            keys.to_vec(),
            ex.columns.clone(),
        )
    }

    fn entity_by_name(&self, name: &str) -> Result<&'a Entity, EntityAccessError> {
        self.by_name
            .get(name)
            .copied()
            .ok_or_else(|| EntityAccessError::UnknownEntity(name.to_string()))
    }
    ...
    fn field_by_name<'e>(
        ...
        entity: &'e Entity,
        name: &str,
    ) -> Result<&'e Field, EntityAccessError> {
        entity
            .fields
            .iter()
            .find(|f| f.name == name)
            .ok_or_else(|| EntityAccessError::UnknownField {
    ...
    fn compile_list(
        ...
        entity: &Entity,
        options: &ListOptions,
    ) -> Result<EntityPlan, EntityAccessError> {
        ...
        for filter in &options.filters {
            match filter {
                Filter::Compare { field, op, value } => {
                    ...
                    query.push((field.clone(), format!("{op}.{value}")));
                }
                Filter::Like { field, pattern } => {
                    query.push((field.clone(), format!("like.{pattern}")));
                }
                Filter::In { field, values } => {
                    query.push((field.clone(), format!("in.{}", values.join(","))));
                }
            }
        }
            ...
            query.push((
                ...
                options
                    ...
                    .map(|sort| match sort.direction {
                        SortDirection::Asc => sort.field.clone(),
                        SortDirection::Desc => format!("-{}", sort.field),
                    })
                    .collect::<Vec<_>>()
                    .join(","),
            ));
        ...
        if !options.expand.is_empty() {
            query.push(("expand".to_string(), options.expand.join(",")));
        }
            ...
            match key.as_str() {
                ...
                "limit" => {
                    let n: u32 = raw.trim().parse().map_err(|_| {
                        EntityAccessError::InvalidRequest(
        ...
        let offset = i64::try_from(offset)
            .map_err(|_| EntityAccessError::InvalidRequest("offset out of range".into()))?;
        ...
        Ok(EntityPlan {
            kind: PlanKind::List,
            statement: Statement::new(sql, pb.params, projection(entity)),
            expands,
        })
    }
    ...
    fn compile_get(
        ...
        require_uuid(id)?;
        let expand_names = expand.to_vec();
        let mut pb = ParamBuilder::new();
    ...
    fn compile_create(
        ...
        let obj = body.as_object().ok_or_else(|| {
            EntityAccessError::InvalidRequest("request body must be a JSON object".into())
        })?;
        ...
        for f in &entity.fields {
            if !f.nullable && f.default.is_none() && !obj.contains_key(&f.name) {
                return Err(EntityAccessError::InvalidValue {
                    field: f.name.clone(),
    ...
    fn compile_update(
        ...
        if !replace && obj.is_empty() {
            return Err(EntityAccessError::InvalidRequest(
    ...
    fn compile_delete(&self, entity: &Entity, id: &str) -> Result<EntityPlan, EntityAccessError> {
        require_uuid(id)?;
        let mut pb = ParamBuilder::new();
        let idp = pb.bind(SqlValue::Uuid(id.to_string()));
        let sql = format!(
        ...
        Ok(EntityPlan {
            ...
            statement: Statement::new(sql, pb.params, vec!["id".to_string()]),
            expands: Vec::new(),
        })
    }
    ...
    fn reject_unknown_keys<'k>(
        ...
        for k in keys {
            ...
            if !entity.fields.iter().any(|f| &f.name == k) {
                return Err(EntityAccessError::UnknownField {
                    entity: entity.name.clone(),
                    field: k.clone(),
                });
            }
    ...
    fn filter_clause(
        ...
        op: &str,
        raw: &str,
    ) -> Result<String, EntityAccessError> {
        ...
        match op {
            ...
            "like" => match &field.field_type {
                ...
                _ => Err(EntityAccessError::InvalidValue {
    ...
    /// Resolve `?expand=` relation names into executable [`Expansion`]s.
    ///
    ...
    ///   malformed catalog ([`EntityAccessError::UnservableRelation`]), not an unknown one.
    /// * `many-to-many` / `hierarchical` are rejected up front
    ///   ([`EntityAccessError::UnsupportedExpansion`]): embedding them needs the join
    ...
    fn resolve_expands(
        ...
        entity: &Entity,
        names: &[String],
    ) -> Result<Vec<Expansion>, EntityAccessError> {
        ...
        for name in names {
            let rel = self
                ...
                .ok_or_else(|| EntityAccessError::UnknownRelation {
                    entity: entity.name.clone(),
                    relation: name.clone(),
                })?;
            ...
            match rel.cardinality {
                Cardinality::ManyToMany => {
                    return Err(EntityAccessError::UnsupportedExpansion {
                        entity: entity.name.clone(),
                        relation: name.clone(),
            ...
            let unservable = || EntityAccessError::UnservableRelation {
                entity: entity.name.clone(),
                relation: name.clone(),
            };
...
/// order preserved. Without this, `?expand=x,x,…` compiles to one [`Expansion`] per
...
fn require_uuid(id: &str) -> Result<(), EntityAccessError> {
    if is_uuid(id) {
        Ok(())
    } else {
        Err(EntityAccessError::InvalidValue {
            field: "id".into(),
...
/// writes.
fn value_for_field(field: &Field, v: &Value) -> Result<SqlValue, EntityAccessError> {
    let err = |m: String| EntityAccessError::InvalidValue {
        field: field.name.clone(),
        message: m,
    };
    if v.is_null() {
...
fn value_for_field_str(field: &Field, s: &str) -> Result<SqlValue, EntityAccessError> {
    let err = |m: String| EntityAccessError::InvalidValue {
        field: field.name.clone(),
        message: m,
    };
