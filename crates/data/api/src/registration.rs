...
//!
//! Style matches the rest of the gateway and `wamn_schema_control::sql`: **identifiers
...
use wamn_pg_core::{SqlValue, Statement as Compiled};
...
pub fn list(catalog_id: &str) -> Compiled {
    Compiled::new(
        "SELECT registration_id, flow_id, entity_id, registration \
              FROM catalog.event_registrations \
              WHERE catalog_id = $1 \
              ORDER BY registration_id ASC"
            .to_string(),
        vec![SqlValue::Text(catalog_id.to_string())],
        read_columns(),
    )
}
