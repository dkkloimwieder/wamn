...
//! predicate `wamn_schema_control`'s `entities_requiring_full` / `pending_old_image_gap`
...
//!
//! Reusing the whole `wamn_schema_control` planner from the wasm guest was avoided on
...
//! engine (`wamn-schema-control` → `wamn-control-registry`) into the component — the least
...
pub fn pending_replica_identity_warning(
    ...
    (reg.requires_replica_identity_full() && !table_is_full).then(|| {
        Warning::PendingReplicaIdentityReconcile {
            entity: reg.entity.to_string(),
        }
    })
}
...
mod tests {
    ...
    fn replica_identity_warning_present_for_delete_on_default_table() {
        ...
        assert_eq!(
            w,
            Some(Warning::PendingReplicaIdentityReconcile {
    ...
    fn replica_identity_warning_absent_when_table_already_full() {
        assert_eq!(
            pending_replica_identity_warning(&reg(vec![Op::Delete], None), true),
            None
        );
    }
