...
//!
//! Turns a project's [`Catalog`] (3.1) into a REST
...
//!   name comes from the [`Catalog`] and is quoted
//!   with [`wamn_pg_core::quote_ident`]; a request string that does not resolve
//!   to a catalog field/relation is rejected ([`ApiError`]) — it never becomes
//!   an identifier.
...
//! # fn demo(catalog: &wamn_schema_model::Catalog) -> Result<(), wamn_api::ApiError> {
...
pub use wamn_entity_access::{attach_expansion, shape_row, shape_rows};
pub use wamn_pg_core::SqlValue;
