...
async fn live_registration_crud_and_rls_isolation() {
    ...
    assert!(
        resp2.get("warnings").is_none(),
        "no warning → no warnings field"
    );
