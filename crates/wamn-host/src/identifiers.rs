...
//!
//! Both the `wamn:postgres` plugin (claim injection) and the flow dispatcher
//! (its pinned per-project session) import these, so a config value is held to
//! the SAME shape on both sides. The pre-R16b divergence was a dispatch-local
...
/// fails closed.
pub(crate) fn valid_runner(runner: &str) -> bool {
    !runner.is_empty()
        && runner.len() <= 128
        && runner
            .chars()
...
pub(crate) fn valid_schema(schema: &str) -> bool {
    !schema.is_empty()
        && schema.len() <= 63
        && schema
            .chars()
