//! RFC 8785 canonical JSON and the graph SHA-256.
...
fn write_value(value: &Value, out: &mut Vec<u8>) {
    match value {
        ...
        Value::Number(number) => {
            ...
            out.extend_from_slice(ecma_number(value).as_bytes());
        }
        Value::String(value) => {
...
/// Format a finite JSON number with ECMAScript `Number::toString` thresholds.
///
/// `serde_json` has already converted the parsed number to IEEE-754. Its
/// shortest-round-trip digits are rearranged here to the fixed/scientific
/// forms required by RFC 8785.
...
pub(crate) fn sha256(input: &[u8]) -> [u8; 32] {
    ...
    let bit_len = u64::try_from(input.len())
        .expect("graph length fits u64")
        .checked_mul(8)
        .expect("graph bit length fits u64");
...
mod tests {
    ...
    use super::{ecma_number, sha256, to_vec};

    #[test]
    fn canonicalizer_uses_utf16_key_order_and_ecmascript_numbers() {
        let value = json!({
            "\u{e000}": 1e-7,
            "\u{1f600}": 1e-6,
            "large": 1e21,
        ...
        assert_eq!(
            ...
            r#"{"fixed":100000000000000000000,"large":1e+21,"negative-zero":0,"😀":0.000001,"":1e-7}"#
        );
