//! Product scenario suites: ordered cases pinned to one flow version.
//!
...
//!
//! The model is pure (serde only): it validates
...
//! ([`crate::TestCase`]). [`TestSuite::validate`] runs the envelope checks
...
pub struct TestSuite {
    /// The envelope format version — must equal [`SUITE_SCHEMA_VERSION`].
...
pub struct CaseEntry {
    ...
    /// against the [`crate::TestCase`] vocabulary by [`TestSuite::validate`]
...
pub enum TestSuiteError {
    ...
    Parse(String),
    /// `schema-version` was not [`SUITE_SCHEMA_VERSION`].
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            TestSuiteError::SchemaVersion { found } => write!(
                f,
                "unsupported test-suite schema-version {found:?} (this build implements {SUITE_SCHEMA_VERSION:?})"
            ),
...
impl TestSuite {
    ...
    /// [`crate::TestCase`] vocabulary. Envelope checks run FIRST, so a
...
mod tests {
    ...
    fn round_trips_through_json() {
        ...
        assert_eq!(
            ...
            "the suite envelope and opaque case bodies must keep their JSON shape"
        );
    ...
    /// scenario-model [`crate::TestCase`] is rejected, naming the offending case-id.
    #[test]
    ...
    /// A valid scenario-model [`crate::TestCase`] body passes validate-on-write.
    #[test]
    fn accepts_a_valid_scenario_case_body() {
        let src = suite_json(json!([
