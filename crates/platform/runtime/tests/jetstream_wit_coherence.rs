...
//! (`docs/contracts/wamn-jetstream.wit`) to the built copy the host bindgen compiles
...
//! kept BYTE-IDENTICAL (like `docs/contracts/wamn-postgres.wit` and its vendored copy).
...
fn docs_wit_matches_built_copy() {
    let docs = fs::read_to_string(root().join("../../../docs/contracts/wamn-jetstream.wit"))
        .expect("docs/contracts/wamn-jetstream.wit reads");
    ...
    assert_eq!(
        docs, built,
        "docs/contracts/wamn-jetstream.wit and the built wit/deps copy drifted — they must \
...
fn component_copies_match_docs() {
    ...
    for copy in [
        ...
        assert_eq!(
            docs, vendored,
            "{copy} drifted from docs/contracts/wamn-jetstream.wit — the vendored guest \
