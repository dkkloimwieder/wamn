...
//!
//! `wamn-execution-contract` and `wamn-router` each declare their own
...
//!
//! wamn-0h0g.26.5 retired the flow language and moved the contract half of the
//! old `wamn-flow` to `wamn-execution-contract/src/ports.rs`. Both declarations
//! survived the move, so this guard did too — it is not corpse coverage, and
//! deleting it removes the only thing that notices the two drifting apart.

#[test]
fn contract_and_router_port_constants_agree() {
    assert_eq!(
        wamn_execution_contract::MAIN_PORT,
        wamn_router::MAIN_PORT,
        "the main-port name is declared twice and the declarations have \
         diverged: components/execution/contract/src/ports.rs:9 \
    ...
    assert_eq!(
        wamn_execution_contract::ERROR_PORT,
        wamn_router::ERROR_PORT,
        "the error-port name is declared twice and the declarations have \
         diverged: components/execution/contract/src/ports.rs:13 \
