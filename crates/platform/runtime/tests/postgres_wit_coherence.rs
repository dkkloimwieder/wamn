...
//!      to `docs/archive/contracts/wamn-postgres.wit`. This is the load-bearing check: the actual
...
//! identical to `docs/archive/contracts/wamn-postgres.wit`: the doc comments diverge (the frozen
...
fn every_copy_shares_the_contract_code() {
    let root = repo_root();
    let docs = fs::read_to_string(root.join("docs/archive/contracts/wamn-postgres.wit"))
        .expect("docs/archive/contracts/wamn-postgres.wit reads");
    ...
    for rel in EXPECTED_COPIES {
        ...
        assert_eq!(
            code_lines(&copy),
            docs_code,
            "{rel} drifted from docs/archive/contracts/wamn-postgres.wit in a CODE line — the vendored \
