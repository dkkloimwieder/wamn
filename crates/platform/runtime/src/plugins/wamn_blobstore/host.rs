...
#[expect(
    ...
    reason = "`HostContainer` is generated by wit-bindgen and declares `async fn`; an \
              implementation that answers without awaiting still has to match it"
)]
...
#[expect(
    clippy::unused_async_trait_impl,
    reason = "`HostContainerWithStore` is generated by wit-bindgen and declares `async fn`; the \
              two verbs that answer from memory still have to match it"
)]
...
#[expect(
    clippy::unused_async_trait_impl,
    reason = "`HostWithStore` is generated by wit-bindgen and declares `async fn`; the four \
              store-level verbs that refuse outright still have to match it"
)]
