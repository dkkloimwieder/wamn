...
/// OCI artifact type and single-layer media type for canonical format-1 bytes.
pub const RELEASE_MANIFEST_ARTIFACT_MEDIA_TYPE: &str =
    "application/vnd.wamn.release-manifest.v1+json";
...
pub struct ReleaseManifestArtifactBlobs<'a> {
    /// The single layer holding the canonical format-1 manifest bytes.
