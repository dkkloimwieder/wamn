    ...
    decode_envelope(input)?
        ...
        .map(|(request_id, body)| {
            let input = match serde_json::from_value::<JsonRequest>(body) {
                Ok(request) => match request.expected_edit_version.parse::<i64>() {
                    Ok(expected_edit_version) => {
                        let request = contract::UpdateRequest {
                            id: request.id,
                            expected_edit_version,
                            change: contract::UpdateChange {
                                code: change(request.change.code),
                                maker_id: change(request.change.maker_id),
                                note: change(request.change.note),
                            },
                        };
                        Ok(request)
                    }
                    Err(_) => Err(invalid("expected_edit_version")),
                },
...
fn normalize(request: &mut contract::UpdateRequest) -> Result<(), contract::InvalidInputDetail> {
    ...
    if matches!(request.change.code, Some(None)) {
        return Err(invalid("change.code"));
    }
    if let Some(Some(value)) = &mut request.change.code
        && (!["priority", "standard"].contains(&value.as_str()))
    {
        return Err(invalid("change.code"));
    }
    if let Some(Some(value)) = &mut request.change.maker_id
        && (!canonical_uuid(value))
    {
        return Err(invalid("change.maker_id"));
    }
    Ok(())
}
...
pub(crate) fn map_error(
    ...
    match code {
        ...
        "unique_violation" => {
            ...
            contract::UpdateError::UniqueViolation(contract::UniqueViolationDetail { constraint })
        }
