...
// The wamn:postgres/statements host calls generated by wit_bindgen are still
// synchronous, so nothing here awaits. The awaited shape is the SDK contract
// callers compile against, and `future::ready` would run the statement at call
// time rather than at await time. The expect drops out on its own once the
// generated host calls become async.
#[expect(
    clippy::unused_async,
    clippy::unused_async_trait_impl,
    reason = "the generated wamn:postgres/statements host calls are not async yet"
)]
impl Connection {
