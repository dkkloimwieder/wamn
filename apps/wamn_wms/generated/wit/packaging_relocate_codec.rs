...
type Item = contract::RelocateItem;
...
pub(crate) fn decode(input: &str) -> Result<Vec<contract::RelocateItem>, CodecError> {
    decode_envelope(input)?
        .into_iter()
        .map(|(request_id, body)| {
            let input = serde_json::from_value::<JsonRequest>(body)
                .map(|request| contract::RelocateRequest {
            ...
            Ok(contract::RelocateItem { request_id, input })
        })
        .collect()
}
...
pub(crate) fn encode(output: &[contract::RelocateOutcome]) -> String {
    let values = output
        .iter()
...
fn error_value(error: &contract::RelocateError) -> Value {
    let (code, detail) = match error {
        contract::RelocateError::InvalidInput(value) => {
        ...
        contract::RelocateError::NotFound(value) => {
            let mut detail = Map::new();
            detail.insert("field".to_owned(), json!(value.field));
            detail.insert("id".to_owned(), json!(value.id));
            ("not_found", detail)
        }
        contract::RelocateError::ConcurrencyConflict(value) => {
            let mut detail = Map::new();
            detail.insert(
        ...
        contract::RelocateError::IdempotencyConflict(value) => {
            let mut detail = Map::new();
            detail.insert("field".to_owned(), json!(value.field));
            ("idempotency_conflict", detail)
        }
        contract::RelocateError::Retry => ("retry", Map::new()),
        contract::RelocateError::Timeout => ("timeout", Map::new()),
        contract::RelocateError::PermissionDenied(value) => {
            let mut detail = Map::new();
            detail.insert("operation".to_owned(), json!(value.operation));
            ("permission_denied", detail)
        }
        contract::RelocateError::InternalError => ("internal_error", Map::new()),
    };
    json!({"code": code, "detail": detail})
}
#[allow(clippy::unnecessary_wraps)]
fn normalize(request: &mut contract::RelocateRequest) -> Result<(), contract::InvalidInputDetail> {
    let _ = &request;
    {
...
pub(crate) async fn run<S, F>(
    input: Vec<contract::RelocateItem>,
    state: &mut S,
    mut handler: F,
) -> Vec<contract::RelocateOutcome>
where
    F: AsyncFnMut(
        &mut S,
        contract::RelocateRequest,
    ...
    for item in input {
        let outcome = match item.input {
            Ok(mut request) => match normalize(&mut request) {
                ...
                Err(error) => Err(contract::RelocateError::InvalidInput(error)),
            },
            Err(error) => Err(contract::RelocateError::InvalidInput(error)),
        };
        output.push(contract::RelocateOutcome {
            request_id: item.request_id,
            outcome,
        });
    }
...
) -> contract::RelocateError {
    match code {
        "invalid_input" => {
            let Some(field) = detail("field") else {
                return contract::RelocateError::InternalError;
            };
            ...
            contract::RelocateError::InvalidInput(contract::InvalidInputDetail {
                field,
                minimum,
                maximum,
                observed,
        ...
        "not_found" => {
            ...
            contract::RelocateError::NotFound(contract::NotFoundDetail { field, id })
        }
        "concurrency_conflict" => {
            ...
            contract::RelocateError::ConcurrencyConflict(contract::ConcurrencyConflictDetail {
        ...
        "idempotency_conflict" => {
            ...
            contract::RelocateError::IdempotencyConflict(contract::IdempotencyConflictDetail {
                field,
            })
        }
        "retry" => contract::RelocateError::Retry,
        "timeout" => contract::RelocateError::Timeout,
        "permission_denied" => {
            let Some(operation) = detail("operation") else {
                return contract::RelocateError::InternalError;
            };
            contract::RelocateError::PermissionDenied(contract::PermissionDeniedDetail {
                operation,
            })
        }
        _ => contract::RelocateError::InternalError,
    }
}
        ...
        const _: () = {
            ...
            impl __contract::Guest for $component {
                async fn run(
                    ...
                    input: Vec<__contract::RelocateItem>,
                ) -> Result<Vec<__contract::RelocateOutcome>, __node::NodeError> {
                    let mut state = $state;
