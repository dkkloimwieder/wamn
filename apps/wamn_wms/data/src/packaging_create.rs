//! `packaging.create` with atomic state and stored replay results.
use crate::error::{self, AccessError, AccessErrorKind};
use crate::generated::wamn::packaging_create as sql;
use crate::scalar;
...
pub async fn execute(command: &CreateCommand) -> Result<CreateResult, AccessError> {
    scalar::text("value.idempotency_key", Some(&command.idempotency_key))?;
    let r#type = scalar::text("value.type", Some(&command.r#type))?.to_owned();
    let code = scalar::text("value.code", Some(&command.code))?.to_owned();
    let location_id = scalar::uuid("value.location_id", &command.location_id)?;
    ...
    .map_err(|e| error::from_statement(&e))?
    .ok_or_else(retry)?;
    sql::validate_location(&mut transaction, location_id.clone())
        .await
        .map_err(|e| error::from_statement(&e))?
        .ok_or_else(|| missing("value.location_id", &location_id.0))?;
    let result_row = sql::apply(
        &mut transaction,
        claim.packaging_id.clone(),
        r#type,
        code,
        location_id,
    )
