use wamn_wms_data_access::inventory_transaction;

use crate::detail;

mod get {
    use super::{detail, inventory_transaction};
    ...
    async fn handle(
        ...
        inventory_transaction::get(connection, &request.id)
            .await
...
mod query {
    ...
    async fn handle(
        ...
        inventory_transaction::query(connection, request.cursor.as_deref(), request.limit)
            .await
