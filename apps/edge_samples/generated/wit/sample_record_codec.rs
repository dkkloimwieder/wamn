...
pub(crate) fn encode(output: &[contract::RecordOutcome]) -> String {
    let values = output
        .iter()
...
fn error_value(error: &contract::RecordError) -> Value {
    let (code, detail) = match error {
        contract::RecordError::InvalidInput(value) => {
        ...
        contract::RecordError::IdempotencyConflict(value) => {
            let mut detail = Map::new();
            detail.insert("field".to_owned(), json!(value.field));
            ("idempotency_conflict", detail)
        }
        contract::RecordError::Retry => ("retry", Map::new()),
        contract::RecordError::Timeout => ("timeout", Map::new()),
        contract::RecordError::PermissionDenied(value) => {
            let mut detail = Map::new();
            detail.insert("operation".to_owned(), json!(value.operation));
            ("permission_denied", detail)
        }
        contract::RecordError::InternalError => ("internal_error", Map::new()),
    };
    json!({"code": code, "detail": detail})
}
#[allow(clippy::unnecessary_wraps)]
fn normalize(request: &mut contract::RecordRequest) -> Result<(), contract::InvalidInputDetail> {
    let _ = &request;
    Ok(())
}
    ...
    ) -> Result<contract::RecordResult, contract::RecordError>,
{
    ...
    for item in input {
        ...
        output.push(contract::RecordOutcome {
            request_id: item.request_id,
            outcome,
        });
    }
    ...
    match code {
        "invalid_input" => {
            let Some(field) = detail("field") else {
                return contract::RecordError::InternalError;
            };
            let minimum = detail("minimum");
            let maximum = detail("maximum");
            let observed = detail("observed");
            contract::RecordError::InvalidInput(contract::InvalidInputDetail {
                field,
                minimum,
                maximum,
                observed,
        ...
        "idempotency_conflict" => {
            ...
            contract::RecordError::IdempotencyConflict(contract::IdempotencyConflictDetail {
                field,
            })
        }
        "retry" => contract::RecordError::Retry,
        "timeout" => contract::RecordError::Timeout,
        "permission_denied" => {
            let Some(operation) = detail("operation") else {
                return contract::RecordError::InternalError;
            };
            contract::RecordError::PermissionDenied(contract::PermissionDeniedDetail { operation })
        }
        _ => contract::RecordError::InternalError,
    }
}
        ...
        const _: () = {
            ...
            impl __contract::Guest for $component {
                async fn run(
                    ...
                    input: Vec<__contract::RecordItem>,
                ) -> Result<Vec<__contract::RecordOutcome>, __node::NodeError> {
                    let mut state = $state;
