...
use crate::{ReceivingApplication, login};
...
/// Start one Receiving-family operator with the shared login and transport flow.
pub async fn run(
    label: &str,
    factory: fn(&str, SessionBinding) -> ReceivingApplication,
    ...
    let result = run_application_with_client(label, binding, client, factory).await;
